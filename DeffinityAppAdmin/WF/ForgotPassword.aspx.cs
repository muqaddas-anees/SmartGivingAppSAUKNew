...
namespace DeffinityAppDev.WF
{
    public partial class ForgotPassword : System.Web.UI.Page
    {
        ...
        public void PasswrdSendToMail(UserMgt.Entity.Contractor con, string PwdLink)
        {
            Email em = new Email();
            string Body = File.ReadAllText(System.Web.HttpContext.Current.Server.MapPath("~/WF/DC/EmailTemplates/PasswordSendingmail.html"));
            Body = Body.Replace("[border]", Deffinity.systemdefaults.GetWebUrl() + System.Configuration.ConfigurationManager.AppSettings["maillogo"]);
            Body = Body.Replace("[Logo]", Deffinity.systemdefaults.GetWebUrl() + System.Configuration.ConfigurationManager.AppSettings["mailboarder"]);
            ...
            em.SendingMail("info@deffinity.com", con.ContractorName + " Password Reset", Body, con.EmailAddress,"");
        }
        ...
        private void Username()
        {
            ...
            using (UserDataContext ud = new UserDataContext())
            {
                ...
                c = new UserMgt.Entity.Contractor();
                c = (from a in ud.Contractors where a.LoginName == txtusername.Text.Trim() select a).FirstOrDefault();
                if (c != null)
                {
                    ...
                    if (ForgotInfo == null)
                    {
                        ...
                        string PwdLink = Deffinity.systemdefaults.GetWebUrl() + "/WF/ResetPasswordNew.aspx?Vid=" + vid.ToString();
                        Fpwd.Userid = c.ID;
        ...
        private void UsernameAndEmail()
        {
            using (UserDataContext ud = new UserDataContext())
            {
                ...
                Fpwd = new ForgotPasswordInfo();
                c = new UserMgt.Entity.Contractor();
                c = (from a in ud.Contractors where a.EmailAddress == txtEmail.Text.Trim() && a.LoginName == txtusername.Text.Trim() select a).FirstOrDefault();
                if (c != null)
                {
                    UserMgt.Entity.ForgotPasswordInfo ForgotInfo = (from a in ud.ForgotPasswordInfos
                                                                    where a.Userid == c.ID && a.IsActive == true
