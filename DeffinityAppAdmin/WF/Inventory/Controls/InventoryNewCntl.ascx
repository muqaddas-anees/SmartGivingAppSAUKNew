<%@ Control Language="C#" AutoEventWireup="True" CodeBehind="InventoryNewCntl.ascx.cs"
             Inherits="DeffinityAppDev.WF.Inventory.Controls.InventoryNew" %>
...
<asp:UpdateProgress ID="UpdateProgress6" runat="server" AssociatedUpdatePanelID="UpdatePanel2">
 ...
 <%--       <script src="../../../ui/jquery-ui.js"></script>
      ...
      <style type="text/css">
          ...
          .mq.js.lt-ie10 .sticky-table-header th[data-priority], .mq.js .table-responsive[data-pattern="priority-columns"] td[data-priority], .mq.js.lt-ie10 .sticky-table-header td[data-priority] {
              display: table-cell;
          }
   ...
   <%-- <script src='<%:ResolveClientUrl("~/Scripts/respond.min.js") %>'></script>
    <script src='<%:ResolveClientUrl("~/Content/assets/js/rwd-table/js/rwd-table.min.js") %>'></script>
    <script src='<%:ResolveClientUrl("~/Scripts/GridDesingFix.js") %>'></script>--%>
...
<script language="javascript" type="text/javascript">
    ...
    function BindBatchesInMainGrid(productid, Cid) {//TblBatchgridNonBatchControl
        $('#TblBatchgrid' + productid).html("");
        $('#TblBatchgridNonBatchControl' + productid).html("");
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.d != "[]") {
                    ...
                    var newData1 = jQuery.parseJSON(data.d);
                    var Headerstext1 = Object.keys(newData1[0]);

                    var trHTML = '';
                    var Headerstext = Object.keys(newData[0]);
                    trHTML += '<tr style="background-color:#40bbea;border:thin;color:white;">';
                    for (var i = 0; i < Headerstext.length; i++) {
                        if (Headerstext[i] != 'BatchID' && Headerstext[i] != 'Batch Control') {//
                    ...
                    $.each(newData, function (i, item) {
                        trHTML += '<tr>';
                        if (item[Headerstext[3]] == "YES") {
                            NumberOfRecords = 1;
                            for (var j = 0; j < Headerstext.length; j++) {
                                if (Headerstext[j] != 'BatchID' && Headerstext[j] != 'Batch Control') {//&& Headerstext[j] != 'Batch Control'
                                    //alert(item[Headerstext[j]]);
                    ...
                    var trHTMLNonBatch = '';
                    trHTMLNonBatch += '<tr style="background-color:#8dc63f;border:thin;color:white;">';
                    for (var i = 0; i < Headerstext.length; i++) {
                        if (Headerstext[i] != 'BatchID' && Headerstext[i] != 'Batch Control' && Headerstext[i] != 'Batch Ref') {
                            trHTMLNonBatch += '<th>' + Headerstext[i] + '</th>';
                        }
                    }
                    trHTMLNonBatch += ' </tr>';
                    var NumberOfRecordsNonBatch = '';
                    $.each(newData, function (i, item) {
                        trHTMLNonBatch += '<tr>';
                        if (item[Headerstext[3]] == "NO") {
                            NumberOfRecordsNonBatch = 1;
                            for (var j = 0; j < Headerstext.length; j++) {
                                if (Headerstext[j] != 'BatchID' && Headerstext[j] != 'Batch Control' && Headerstext[j] != 'Batch Ref') {
                                    if (item[Headerstext[j]] != null) {
                                        trHTMLNonBatch += '<td>' + item[Headerstext[j]] + '</td>';
                                    }
                                    else {
                                        trHTMLNonBatch += '<td></td>';
                                    }
                                }
                            }
                        }
                    ...
                    if (NumberOfRecordsNonBatch != '') {
                        $('#TblBatchgridNonBatchControl' + productid).append(trHTMLNonBatch);
                    }
                    else {
                        $('#TblBatchgridNonBatchControl' + productid).append("<tr><td>No data<td></tr>");
                    }


                    //Width setting
                    var MainGrid = document.getElementById("DivInventoryMainGrid");
                    var InnerGrid1 = document.getElementById("div" + productid);
                    if (MainGrid != null && InnerGrid1 != null) {
                        InnerGrid1.style.width = MainGrid.clientWidth - 30 + "px";
                    }
                }
                else {
                    $('#TblBatchgrid' + productid).append("<tr><td>No data<td></tr>");
            ...
            function Close_All(obj) {
                ...
                var getAttribute;
                var str = '';
                var Grid_Table = document.getElementById('<%= grdInventoryNew.ClientID %>');
...
<script>
    ...
    function ErrorMagWithStyle() {

        var SuccessMessageDiv = "<div class='alert alert-success'></div>";
        var ErrorMessageDiv = "<div class='alert alert-danger'></div>";

        var LblNames = ["#lblInventoryAddMsg", "#lblUploadErrorMsg",
          "#LblListviewMsg", "#lblTransferMsg", "#lblmsgInpopup",
          "#LblMsgInReclaime", "#ValReq1", "#lblInventoryIdTodeleteMsg"];

        for (var i = 0; i < LblNames.length; i++) {
            var lblColor = $(LblNames[i]).css('color');
            var lbldisplay = $(LblNames[i]).css('display');

            if (lbldisplay != 'none') {
                if (lblColor == "rgb(0, 128, 0)") {
                    $(LblNames[i]).wrap(SuccessMessageDiv);
                    $(LblNames[i]).css("color", 'white');
                }
                else if (lblColor == "rgb(255, 0, 0)") {
                    $(LblNames[i]).wrap(ErrorMessageDiv);
                    $(LblNames[i]).css("color", 'white');
                }
            }
        }
    ...
    function InIEvent() {
        ...
        $("#BtnReClaimeSubmit").click(function () {
            ErrorMagWithStyle();
        });
        ErrorMagWithStyle();
    }
                function clickingfirstone() {
                    //clicking first one
                    var FristId = $("#InventoryFirstId").val();
                    // alert(FristId);
                    if (FristId != '') {
                        expandcollapseInnerGrid('divInner' + FristId + '', 'one');
                    }
                }
</script>
<script type="text/javascript">
    ...
    Sys.WebForms.PageRequestManager.getInstance().add_endRequest(clickingfirstone);
</script>
 ...
 <%--   <script type="text/javascript">
        ...
        <div class="form-group">
             <div class="col-md-12" style="float:right;text-align:right;">
                   <asp:LinkButton ID="UploadImg" Text="Upload" SkinID="BtnLinkUpload1" runat="server" ClientIDMode="Static"></asp:LinkButton>
                   <asp:LinkButton ID="hlinkExecption" runat="server" OnClick="hlinkExecption_Click" 
                                                                      ClientIDMode="Static" SkinID="BtnLinkException"></asp:LinkButton>
            </div>
        </div>
                ...
                <div class="form-group">
                       ...
                       <div class="col-md-4">
                               <label class="col-sm-4 control-label"> <%= Resources.DeffinityRes.SubCategory%></label>
                             	<div class="col-sm-8">
                                      <asp:DropDownList ID="ddlsubcategoryInSearch" AutoPostBack="true"
                                           SkinID="ddl_80" runat="server" OnSelectedIndexChanged="ddlProductInSearch_SelectedIndexChanged"></asp:DropDownList>
                             	</div>
                         </div>
                    <div class="col-md-4">
                        <label class="col-sm-4 control-label"><%= Resources.DeffinityRes.Description%></label>
                        <div class="col-sm-8">
                              <asp:DropDownList ID="ddlProductInSearch" runat="server" SkinID="ddl_70"
                   ...
                   <div class="form-group">
                      ...
                      <div class="col-md-4">
                               <label class="col-sm-4 control-label"> <%= Resources.DeffinityRes.Text%></label>
                             	<div class="col-sm-8">
                                     <asp:TextBox ID="txtText" runat="server" SkinID="txt_80"></asp:TextBox>
                             	</div>
                         </div>
                         <div class="col-md-4">
                      ...
                      <div class="col-md-4">
                          <div class="col-sm-12">
                              ...
                              <asp:LinkButton ID="btnGetReport" runat="server"
                                    SkinID="BtnLinkSUMMARYREPORT" OnClick="btnGetReport_Click"></asp:LinkButton>
                    ...
                    <div class="form-group">
                             ...
                             <div class="form-group">
                                ...
                                SetFocusOnError="True" Type="Integer" ValidationGroup="Group2"></asp:CompareValidator>
                                  ...
                                  <div class="form-group">
                                <div class="col-sm-4">
                                        <asp:Button ID="btnaddnewstock"
                                             runat="server" CausesValidation="false" Text="Add Stock" OnClick="btnaddnewstock_Click" />
                                </div>
                                      </div>
                          </div>
                             <div class="col-md-4">
...
<div class="row">
        ...
        <asp:Panel ID="pnlHistory" runat="server" BackColor="White" Style="display: none;"
         ...
         <ajaxToolkit:ModalPopupExtender ID="mdlpopupaddnewstock" runat="server" BackgroundCssClass="modalBackground"
              TargetControlID="LblForbtnaddnewstock" PopupControlID="paneladdstock"></ajaxToolkit:ModalPopupExtender>
     <asp:Label ID="LblForbtnaddnewstock" runat="server"></asp:Label>
                    ...
                    <asp:Label ID="imgloading_4" runat="server" SkinID="Loading"></asp:Label>
                           ...
                           <asp:UpdatePanel ID="popupStockAdd" runat="server" ClientIDMode="Static" UpdateMode="Conditional">
                                              <ContentTemplate>
                                                  <div>
                                                       <div class="col-md-12">
                                                           <asp:Label ID="lblmsgInpopup" EnableViewState="false" ClientIDMode="Static" runat="server"></asp:Label>
	...
	<div class="col-md-4">
          ...
          <div class="col-md-4">
           ...
           <div class="col-sm-9">
               <asp:TextBox ID="txtnotesinpopup" runat="server"></asp:TextBox>
                                                           ...
                                                           <div class="col-md-4">
                                                                ...
                                                                <div class="col-sm-6">
                                                                    <asp:RadioButtonList ID="BatchControlRblist"
                                                                         runat="server" AutoPostBack="true"
                                                                         OnSelectedIndexChanged="BatchControlRblist_SelectedIndexChanged"
           ...
           <div class="col-md-4">
           <div class="col-sm-9">
                <asp:Button ID="btnSubmitInPopUp" runat="server" Text="Submit" Visible="true"
                        ValidationGroup="AddStock" OnClick="btnSubmitInPopUp_Click"/>
                <asp:Button ID="BtnUpdateBatchDetails" runat="server" Visible="false"
                     ValidationGroup="AddStock" Text="Update" OnClick="BtnUpdateBatchDetails_Click" />

            </div>
	</div>
                                                      </div>
                                                    <div class="form-group" id="PlaceHolderDivInAddItems" runat="server" visible="false">
                                                        <div class="col-md-12">
                                                               <div class="col-sm-12">
                                                                    <div class="form-group">
                                                                           <div class="col-md-12">
                                                                                 <asp:Panel ID="Panel1" runat="server" CssClass="tab_subheader"> 
                                                                   <b>Batch Reference Details</b>   </asp:Panel>
                                                                 <hr class="no-top-margin" /></div>
                                                                     </div>
                                                                      <asp:Panel ID="Panel2" runat="server">
                                                                          <asp:PlaceHolder
                                                                         ID="PlaceHolderBatchFiledsInAddStock" runat="server"></asp:PlaceHolder>
                                                                      </asp:Panel>
                ...
                <asp:TextBox ID="txtNote" runat="server" TextMode="MultiLine"></asp:TextBox>
                              ...
                              <div class="col-sm-4">
                                     <asp:CheckBox ID="ChcekFromBacthControl" runat="server"
                                         AutoPostBack="true" Text="Take quantity from non-batch related stock"
                                          OnCheckedChanged="ChcekFromBacthControl_CheckedChanged" />
                              </div>
	</div>
                    </div>
               ...
               <asp:Panel ID="pnl1" runat="server" Width="100%">
                        ...
                        <asp:Panel ID="PnlListview" runat="server" Visible="false" Width="100%">
                            ...
                            <div style="float:right;">
                               ...
                               <asp:Button ID="btnFieldConfigurator" runat="server"
                                    Text="Field Configurator" OnClick="btnFieldConfigurator_Click" Visible="false" />
                                </div>
                             <div>
...
<hr class="no-top-margin" />
             ...
             <asp:Panel ID="PanelForDeleteOptions" runat="server" BackColor="White"
                        Style="display:none;" Width="300px" Height="90" BorderStyle="Double" BorderColor="LightSteelBlue" ScrollBars="None">
                 <div class="form-group">
                     <div class="col-md-12" style="text-align:center;">
                          <b>Do you want to delete this inventory ?</b> 
                     </div>
                 </div>
                 <div class="form-group">
                     <div class="col-md-12" style="text-align:center;">
            ...
            <ajaxToolkit:ModalPopupExtender ID="MdlPopUpForDeleteOption" runat="server" BackgroundCssClass="modalBackground"
                             TargetControlID="l11Delete" PopupControlID="PanelForDeleteOptions"></ajaxToolkit:ModalPopupExtender>
            <asp:Label ID="l11Delete" runat="server"></asp:Label>
...
<asp:Panel ID="pnlInventory" runat="server" ClientIDMode="Static" Width="100%">
                        ...
                        <asp:HiddenField ID="InventoryFirstId" runat="server" ClientIDMode="Static" />
                                  <div style="float:right;text-align:right;margin-bottom:3px;">
                                        <asp:LinkButton ID="lnKBtnShowAllColumns" Text="Show all Columns" Visible="false"
                                           runat="server" CssClass="btn btn-gray" CausesValidation="false"
                                           OnClick="lnKBtnShowAllColumns_Click"></asp:LinkButton>
    ...
    <div id="DivInventoryMainGrid" style="overflow:no-display;width:100%;">
    <asp:Label ID="lblInventoryIdTodelete" runat="server" Visible="false"></asp:Label>
        <asp:Label ID="lblInventoryIdTodeleteMsg" runat="server" ClientIDMode="Static" EnableViewState="false"></asp:Label>

    <asp:GridView ID="grdInventoryNew" runat="server" AutoGenerateColumns="false" AllowPaging="true" PageSize="10"
         OnRowDataBound="grdInventoryNew_RowDataBound" ClientIDMode="Static" Width="100%" OnPageIndexChanging="grdInventory_PageIndexChanging"
         OnRowCommand="grdInventoryNew_RowCommand" EmptyDataText="No products available">
        <Columns>
         <asp:TemplateField ItemStyle-CssClass="col-nowrap" ItemStyle-Width="50px"  ControlStyle-Width="50px" FooterStyle-Width="50px">
             <ItemStyle HorizontalAlign="Center" />
             <ItemTemplate>
                 <a id="imageColapse123" href="javascript:expandcollapseInnerGrid('divInner<%# Eval("id") %>','one');">
                     <img title='View History' id='imgdivInner<%# Eval("id") %>'
                 ...
                 <asp:LinkButton ID="LinkButtonEdit" runat="server" CausesValidation="false"
                     CommandArgument='<%# Bind("id")%>' Visible="false"
                     CommandName="MoreOptions" SkinID="BtnLinkEdit" ToolTip="<%$ Resources:DeffinityRes,Edit%>"></asp:LinkButton>
                 <asp:Label ID="lblGridInventoryId" runat="server" Text='<%#Bind("ItemDescription") %>' Visible="false"></asp:Label>
             </ItemTemplate>
             <EditItemTemplate>
              ...
              <asp:TemplateField ItemStyle-CssClass="col-nowrap" ItemStyle-Width="100px"  ControlStyle-Width="100px" FooterStyle-Width="100px">
                                    <ItemTemplate >
                                ...
                                <asp:TemplateField ItemStyle-CssClass="col-nowrap" ItemStyle-Width="125px"  ControlStyle-Width="125px" FooterStyle-Width="125px"
                                     HeaderText="<%$ Resources:DeffinityRes,Category%>" SortExpression="CategoryName">
                                    <ItemTemplate>
                                        <asp:Label ID="lblCategory" runat="server" Text='<%# Bind("CategoryName") %>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField ItemStyle-CssClass="col-nowrap" ItemStyle-Width="150px"  ControlStyle-Width="150px" FooterStyle-Width="150px"
                                     HeaderText="<%$ Resources:DeffinityRes,SubCategory%>" SortExpression="SubCategoryName">
                                    <ItemTemplate>
                                        <asp:Label ID="lblSubCategory" runat="server" Text='<%# Bind("SubCategoryName") %>'> 
                                        </asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField ItemStyle-CssClass="col-nowrap" ItemStyle-Width="300px"  ControlStyle-Width="300px" FooterStyle-Width="300px"
                                     HeaderText="<%$ Resources:DeffinityRes,Description%>">
                                    <ItemTemplate>
                                         <a id="imageColapse123" style="color:green;"
                                              href="javascript:expandcollapseInnerGrid('divInner<%# Eval("id") %>','one');">
                                            <asp:Label ID="lblDesc" runat="server" Text='<% #Bind("ItemDescription")  %>'></asp:Label>
                                         </a>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                <asp:TemplateField HeaderText="<%$ Resources:DeffinityRes,Manufacturer%>" Visible="false"
                                     SortExpression="ManufacturerName" ItemStyle-CssClass="col-nowrap" ItemStyle-Width="125px"  ControlStyle-Width="125px" FooterStyle-Width="125px">
                                    <ItemTemplate>
                                        <asp:Label ID="lblManufcturer" runat="server" Text='<%# Bind("ManufacturerName") %>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
            <asp:TemplateField ItemStyle-CssClass="col-nowrap" ItemStyle-Width="75px"  ControlStyle-Width="75px" FooterStyle-Width="75px"
                 HeaderText="<%$ Resources:DeffinityRes,QTYAvailable%>">
                   <ItemTemplate>
                       <asp:Label ID="lblQTYAvailable" runat="server"
                           Text='<%# getActualStackLaevelInMainGridWithItemDescription(Eval("ItemDescription").ToString()) %>'></asp:Label>
                   </ItemTemplate>
                ...
                <ItemTemplate>
                    <tr>
                       <td colspan="100%" style="padding:0px">
                               <div id='divInner<%# Eval("id") %>' 
                                   style="display:none; position: relative; left: 15px;padding-bottom:5px;padding-top:15px;">
                                   <asp:GridView ID="grdInventory" runat="server" AllowPaging="false" AutoGenerateColumns="False"
                            DataKeyNames="ID" OnPageIndexChanging="grdInventory_PageIndexChanging" PageSize="20"
                            ...
                            <Columns>
                                <asp:TemplateField HeaderText="Edit">
                                    <ItemTemplate>
                                        <asp:LinkButton ID="LinkButtonEdit" runat="server" CausesValidation="false"
                                            CommandArgument='<%# Bind("id")%>' Visible="true"
                                            CommandName="MoreOptions" SkinID="BtnLinkEdit"></asp:LinkButton>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                 <asp:TemplateField
                                      ItemStyle-CssClass="col-nowrap" ItemStyle-Width="300px"  ControlStyle-Width="300px" FooterStyle-Width="300px"
                                      HeaderText="<%$ Resources:DeffinityRes,Description%>">
                                    <ItemTemplate>
                                        <asp:Label ID="lblDesc" runat="server" Text='<% #Bind("ItemDescription")  %>'></asp:Label>
                                    </ItemTemplate>
                                ...
                                <asp:TemplateField HeaderText="Shelf" Visible="false">
                                    <ItemTemplate>
                                            <asp:Label ID="lblShelfName" runat="server" Text='<%# Bind("ShelfName") %>'></asp:Label>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                ...
                                <asp:TemplateField HeaderText="QTY<br/>Available<br/>(Total)">
                                    <ItemTemplate>
                                        <asp:Label ID="lblQTYAvailable" runat="server"
                                  ...
                                  <asp:TemplateField HeaderText="QTY<br/>Available<br/>(Not Batch Related)" ItemStyle-HorizontalAlign="Right">
                                    <ItemTemplate>
                                ...
                                <asp:TemplateField HeaderText="<%$ Resources:DeffinityRes,Notes%>" Visible="false" >
                                  ...
                                  <asp:TemplateField>
                                    <ItemTemplate>
                                       <asp:LinkButton ID="lnkUsage" runat="server" CausesValidation="false" Text="Deploy Stock" CommandName="Usage"
                                                                                       CommandArgument='<%# Bind("id")%>'></asp:LinkButton>
                                              &nbsp;
                                        <a id="imageColapse"
                                             href="javascript:expandcollapseUsage('divUsage<%# Eval("id") %>','<%# Eval("id") %>','<%# Eval("Cid") %>','one');">

                                              <i title="Deploy Stock" style="width:9px;border:0px;font-size:1.2em;"
                                                   id="imgdivUsage<%# Eval("id") %>" class="fa fa-search-plus"></i>

                                             <%-- <img title='Deploy Stock' id='imgdivUsage<%# Eval("id") %>' class="fa-search-plus"
                                                   alt='Click to show/hide <%# Eval("id") %>' style="width:9px;border:0px" src='plus.gif' />--%>
                                        </a>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                 <asp:TemplateField HeaderText="">
                                    <ItemTemplate>
                                        ...
                                        <div id="GridBtnInMainGrid" runat="server">
                                           ...
                                           &nbsp;
                                            <a id="imageColapse"
                                                 href="javascript:expandcollapse('div<%# Eval("id") %>','<%# Eval("id") %>','<%# Eval("Cid") %>','one');">

                                                   <i title="View Stock Levels" style="width:9px;border:0px;font-size:1.2em;"
                                                   id="imgdiv<%# Eval("id") %>" class="fa fa-search-plus"></i>

                                                <%--<img title='View Stock Levels' 
                                                    id='imgdiv<%# Eval("id") %>' alt='Click to show/hide <%# Eval("id") %>' style="width:9px;border:0px" src='plus.gif' />--%>
                                            </a>
                                        </div>
                                    </ItemTemplate>
                                </asp:TemplateField>
                                ...
                                <asp:TemplateField>
                                    ...
                                    <ItemTemplate>
                                        <asp:LinkButton ID="LinkButtonDelete1" runat="server"
                                            CommandName="Delete" CommandArgument='<%# Bind("ID")%>'
                                             SkinID="BtnLinkDelete" ToolTip="<%$ Resources:DeffinityRes,Delete%>">
                                        </asp:LinkButton>
                                    </ItemTemplate>
                                    <ItemStyle HorizontalAlign="Center"/>
                                </asp:TemplateField>
                                    ...
                                    <ItemTemplate>
                                        <tr>
                                            <td colspan="90%" style="padding:0px">
                                                 <div id='divUsage<%# Eval("id") %>'
                                                      style="display:none; position: relative; left: 15px;padding-bottom:5px;overflow: auto; width: 1220px;">
                                                    <strong>Deployed Stock</strong>    
                                                ...
                                                <div id='div<%# Eval("id") %>'
                                                     style="display:none; position: relative; left: 15px;overflow:auto;">
                                                    <strong>History of Stock Added(Batch Control) </strong>
                                                     <div style="color:green;" id='DivPleaseWaitMsgInBatch<%# Eval("id") %>'></div>
                                                     <table style="width:100%;border-collapse: collapse;font-size:smaller;border-color:whitesmoke;"
                                                          class="table responsive"
                                                          id="TblBatchgrid<%# Eval("id")%>" border="1"></table>
                                                    <strong>History of Stock Added(Non-Batch Control) </strong>
                                                    <table style="width:100%;border-collapse: collapse;font-size:smaller;border-color:whitesmoke;"
                                                          class="table responsive"
                                                          id="TblBatchgridNonBatchControl<%# Eval("id")%>" border="1"></table>
                                                     </div>
                                            </td>
                                        </tr>
                                    </ItemTemplate>
    ...
    <Triggers>
        ...
        <asp:PostBackTrigger ControlID="grdInventoryNew" />
        <asp:PostBackTrigger ControlID="btnSubmitInPopUp" />
