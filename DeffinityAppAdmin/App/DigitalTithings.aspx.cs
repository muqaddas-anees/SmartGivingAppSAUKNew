...
{
    public partial class DigitalTithing : System.Web.UI.Page
    {
            ...
            public void BingGrid(bool getNewData = false)
            {
                ...
                var listTithingDefaults = pRep.GetAll().Where(o => o.OrganizationID > 0).ToList();
                
                GridInstances.DataSource = listTithingDefaults;
            ...
            protected void btnAddOrganization_Click(object sender, EventArgs e)
            {
                try
                {
                    Response.Redirect("~/App/DigitalTithing.aspx", false);
                }
                catch (Exception ex)
                {
                    LogExceptions.WriteExceptionLog(ex);
