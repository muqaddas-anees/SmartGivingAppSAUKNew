...
public class GridViewHelper
{
    ...
    private GridViewRow InsertGridRow(GridViewRow beforeRow, GridViewGroup g)
    {
        ...
        else
        {
            ...
            {
                cell = new TableCell();
                cell.Text = SqlHelper.ExecuteScalar(Constants.DBString, CommandType.Text, "Select ShortCurrencyName from currencylist where ID =(SELECT DefaultCurrency from ProjectDefaults)").ToString();
                //cell.Text = "&nbsp;";
                cell.ColumnSpan = colspan;
                tcc.Add(cell);
                colspan = 0;
