    ...
    public class ContactController : ApiController
    {
           ...
           .Db("files", "id", new Dictionary<string, object>
           {
                ...
                //{"systemPath", DataTables.Upload.DbType.SystemPath}
                   ...
                   //)
                        ...
                        .LeftJoin("PortfolioContacts as PortfolioContact", "PortfolioContact.ID", "=", "PortfolioContactAddress.ContactID")
                      //.LeftJoin("AssetAssociatedToContacts", "AssetAssociatedToContacts.ContactAddressID", "=", "PortfolioContactAddress.ID")
                      //
                      //.Where(p => p.Where("PortfolioContactAddress.ContactID", ContactID, "="))
                ...
                if (!string.IsNullOrEmpty(AssetID))
                {
                    ...
                    var response = new Editor(db, "Assets", "ID")
                        ...
                        .Field(new Field("Assets.Type")
                            .Options(new Options()
                                .Table("DC.Categories")
                                .Where(q => q.Where("DC.Categories.ID", "(select Type from Assets)", "IN", false))
                                .Value("ID")
                                .Label("Name")
                        ...
                        .Field(new Field("Assets.Make")
                            .Options(new Options()
                                .Table("DC.SubCategory")
                                .Where(q => q.Where("DC.SubCategory.ID", "(select Make from Assets)", "IN", false))
                                .Value("ID")
                                .Label("Name")
                            )
                         ...
                         .Field(new Field("Assets.Model")
                            .Options(new Options()
                                .Table("DC.ProductModel")
                                .Where(q => q.Where("DC.ProductModel.ModelID", "(select Model from Assets)", "IN", false))
                                .Value("ModelID")
                                .Label("ModelName")
                            )
                        // .Validator(Validation.DbValues(new ValidationOpts { Empty = false }))
           ...
           .Db("files", "id", new Dictionary<string, object>
           {
                ...
                //{"systemPath", DataTables.Upload.DbType.SystemPath}
                   ...
                   //})
                        ...
                        .LeftJoin("PortfolioContacts as PortfolioContact", "PortfolioContact.ID", "=", "PortfolioContactAddress.ContactID")
                       //.LeftJoin("AssetAssociatedToContacts", "AssetAssociatedToContacts.ContactAddressID", "=", "PortfolioContactAddress.ID")
                       .Where(p => p.Where("Assets.ID", AssetID, "=").Where("PortfolioContact.PortfolioID",sessionKeys.PortfolioID,"="))
       ...
       new DataTables.Upload((file, id) =>
       {
           ...
           //)
                      ...
                      //.LeftJoin("AssetAssociatedToContacts", "AssetAssociatedToContacts.ContactAddressID", "=", "PortfolioContactAddress.ID")
                      //ContactAddressID
                      .Where(p => p.Where("PortfolioContactAddress.ID", ContactAddressID, "=").Where("PortfolioContact.PortfolioID", sessionKeys.PortfolioID, "="))
                      //.Where(p => p.Where("PortfolioContactAddress.ContactID", ContactID, "="))
                      //.Where(p => p.Where("Assets.ContactAddressID", ContactAddressID, "="))
                     // .Where(q => q.Where("Assets.ContactAddressID", "(select AssetId from AssetAssociatedToContacts where ContactAddressID =" + ContactAddressID + " )", "IN", false))
                      .Process(request)
        ...
        [Route("api/ContactAdressAssetsByJob")]
        [HttpGet]
        [HttpPost]
        public IHttpActionResult ContactAdressAssetsByJob()
        {
        ...
        public string WSGetPolicyno(int policytypeid)
        {
            ...
            using (PortfolioMgt.DAL.PortfolioDataContext pd = new PortfolioMgt.DAL.PortfolioDataContext())
            {
                using (DC.DAL.DCDataContext dc = new DC.DAL.DCDataContext())
                {
                    ...
                    {
                        //policy type prefix
                        var ptdata = pd.ProductPolicyTypes.Where(o => o.CustomerID == sessionKeys.PortfolioID).Where(o => o.ID == Convert.ToInt32(policytypeid.ToString())).FirstOrDefault();
                        var ptcontact = pd.PortfolioContacts.Where(o => o.PortfolioID == sessionKeys.PortfolioID).ToList();
                        var ptaddress = pd.PortfolioContactAddresses.Where(o=> ptcontact.Select(p=>p.ID).Contains(o.ContactID) ).OrderByDescending(o => o.ID).FirstOrDefault();

                        if (pdata == null)
                        {
                            pdata = new PolicyNumberFormatSetting();
                            pdata.Seed = 100000;
                        ...
                        {
                            if (ptaddress == null)
                                retval = string.Format("{0}{1}", ptdata.Title.Substring(0,2), (pdata.Seed).ToString("D6"));
                            //retval = string.Format("{0} - {1} - {2}", pdata.Prefix, ptdata.PolicyTypePrefix, (pdata.Seed).ToString("D6"));
                            else
                            {
                                // ptdata.PolicyTypePrefix
                                int pno = 0;
                                string policyno =  ptaddress.PolicyNumber== null? "0" : (!string.IsNullOrEmpty(ptaddress.PolicyNumber)? ptaddress.PolicyNumber : "0") ;
                                policyno = ptaddress.PolicyNumber == null ? "0" : ptaddress.PolicyNumber.Replace(ptdata.Title.Substring(0, 2), "");
                                //string[] split = policyno.Split('-').ToArray();
                                //if (split.Count() > 2)
                                //{
                                try
                                ...
                                // }
                                retval = string.Format("{0}{1}", ptdata.Title.Substring(0, 2), (pno + pdata.Seed).ToString("D6"));
                            }
                        }
                    }
                }
