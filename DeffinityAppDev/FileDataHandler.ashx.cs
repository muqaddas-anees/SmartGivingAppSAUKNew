...
{
    /// <summary>
    /// Summary description for FileDataHandler
    /// </summary>
    public class FileDataHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        private byte[] GetImageDataFromDatabase(string imageId, string section, string type, out string ctype,out string filename)
        {
            ...
            if (f != null)
            {
              //  imageData = f.FileData1.ToArray();
                ...
                if (type.Length > 0)
                {
                   ...
                   // string decodedString = Encoding.UTF8.GetString(f.FileData2.ToArray());
                }
                else
                {
                    imageData = f.FileData1.ToArray();
            ...
            else
            {
                ...
                if (type.Length > 0)
                {
                    ...
                    // var str = HttpContext.Current.Server.HtmlDecode*
                    string decodedString = Encoding.UTF8.GetString(f.FileData2.ToArray());
                }
                else
                {
                    imageData = f.FileData1.ToArray();
                    string decodedString = Encoding.UTF8.GetString(f.FileData1.ToArray());
                }
