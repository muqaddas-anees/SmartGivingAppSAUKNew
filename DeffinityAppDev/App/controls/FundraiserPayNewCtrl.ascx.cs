...
{
    public partial class FundraiserPayNewCtrl : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    if (tithing != null)
                    {
                        sessionKeys.PortfolioID = tithing.OrganizationID.HasValue ? tithing.OrganizationID.Value : 0;
        ...
        public string GetAmounts()
        {
            ...
            var tithing = PortfolioMgt.BAL.TithingDefaultDetailsBAL.TithingDefaultDetailsBAL_Select().Where(o => o.unid == QueryStringValues.UNID).FirstOrDefault();
            var amountlist = PortfolioMgt.BAL.TithingAmountBAL.TithingAmountBAL_SelectAll().Where(o => o.TithingDefaultUnid == QueryStringValues.UNID).ToList();

            foreach (var p in amountlist)
            {
                var val = string.Format("{0:C}", p.Amount);
