...
namespace DeffinityAppDev.WF.Admin
{
    public partial class ServiceProviders : System.Web.UI.Page
    {
        ...
        private List<int> GetUserListByCompany()
        {
            ...
            try
            {
                if (Session["CompanyID"] != null)
                {
                    ...
                    var uEntity = uRep.GetAll().Where(o => o.CompanyID == Convert.ToInt32(Session["CompanyID"].ToString())).ToList();
                    if (uEntity.Count > 0)
                ...
                using (UserDataContext Udc = new UserDataContext())
                {
                    ...
                    var Result = (from a in Udc.Contractors
                                  ...
                                  select new
                                  {
                                      ...
                                      SID = a.SID,
                                      UsernameRole =
                                      (a.SID == 1) ? "Administrator" :
                                      (a.SID == 2) ? "Service Managers" :
                                      (a.SID == 3) ? "Project Manager & QA" :
                                      (a.SID == 4 || a.SID == 9) ? "Service Providers" :
                                      (a.SID == 5) ? "QA/UAT" :
                                      (a.SID == 6) ? "Dashboard" :
                    ...
                    else
                    {
                        ...
                        {
                            Result = Result.Where(a => a.UsernameRole == "Administrator").ToList();
                        }
                        ...
                        {
                            Result = Result.Where(a => a.UsernameRole == "Service Providers").ToList();
                        }
                        else if (Type == "QA/UAT")
                        {
        ...
        protected void GridUsers_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "sp")
            {
                Response.Redirect("~/WF/Admin/UserManagement.aspx?Type=ServiceProviders");
            }
        ...
        protected void BtnCreateNewUser_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request.QueryString["Type"] != null)
                {
                    ...
                    else if (Q_String == "ServiceProviders")
                    {
                        Type = 4;
                        Response.Redirect("~/WF/Admin/AdminUsers.aspx?Type=" + Type.ToString(), false);
                    }
