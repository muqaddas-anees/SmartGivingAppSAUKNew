...
{
    public partial class DCAssignResource : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    if (QueryStringValues.CCID > 0)
                        lblTitle.InnerText = "Smart Tech - Job Reference " + QueryStringValues.CCID.ToString() + " : " + FLSDetailsBAL.GetJobDetails(QueryStringValues.CallID); 
                    else
                        lblTitle.InnerText = "Smart Tech";
                ...
                if (rids.Length > 0)
                {
                    ...
                    using (UserMgt.DAL.UserDataContext Pdc = new UserMgt.DAL.UserDataContext())
                    {
                        ...
                        foreach (var c in userlist)
                        {
                            ...
                            cbody = body;
                            //update to associated callids
                            smail = UpdateAssignResources(fd, cd, blastValues, tocheck, smail, cnt, c,false);
                            cbody = cbody.Replace("[resourcename]", c.ContractorName);
        ...
        private static bool UpdateAssignResources(FLSDetail fd, CallDetail cd, ServiceProviderScheduling blastValues, bool tocheck, bool smail, int cnt, UserMgt.Entity.Contractor c,bool IsAssigned)
        {
            using (DCDataContext Dc = new DCDataContext())
            {
                var cdResource = Dc.CallResourceSchedules.Where(o => o.CallID == cd.ID && o.ResourceID == c.ID && o.UserType != CallResourceScheduleBAL.Usertype_SmartRep).FirstOrDefault();
                if (cdResource == null)
                {
        ...
        protected void btnAssign_Click(object sender, EventArgs e)
        {
            ...
            if (hrid.Value != string.Empty)
            {
                ...
                Response.Redirect("~/WF/DC/DCAssignResource.aspx?CCID="+ QueryStringValues.CCID.ToString()+"&callid=" + cid.ToString() + "&SDID=" + cid.ToString());
            }
        ...
        protected void btnReopen_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Response.Redirect("~/WF/DC/DCAssignResource.aspx?CCID=" + QueryStringValues.CCID.ToString() + "&callid=" + QueryStringValues.CallID + "&SDID=" + QueryStringValues.CallID);
            }
            catch (Exception ex)
            { LogExceptions.WriteExceptionLog(ex); }
