...
{
    public partial class TasksCtrl : System.Web.UI.UserControl
    {
        ...
        public void BindListView()
        {
            try
            {
                var retEmpty = new List<JobTargetAssignedUser>();
                var slist = JobTargetStatusBAL.GetStatusList();
                List<JobTarget> rlist = new List<JobTarget>();
                if (Convert.ToInt32(hcallid.Value) > 0)

                    rlist = JobTargetBAL.JobTargetBAL_SelectAll().Where(o => o.CallID == Convert.ToInt32(hcallid.Value)).ToList();
                else
                    rlist = JobTargetBAL.JobTargetBAL_SelectAll().ToList();
                ...
                var calllist = CallDetailsBAL.CallDetails_selectall().Where(o => o.CompanyID == sessionKeys.PortfolioID).ToList();

                var result = (from r in rlist
                              join c in calllist on r.CallID equals c.ID
                              select new
                              {
                                  r.ID,
                                  r.Title,
                ...
                if(Convert.ToInt32( ddlSearchUsers.SelectedValue) >0)
                {
                    result = result.Where(o => o.Users != null?( o.Users.Where(p => p.UserID == Convert.ToInt32(ddlSearchUsers.SelectedValue)).Count() > 0):false).ToList();
                }


                if (txtduedate.Text.Trim().Length >0)
                {
                    var dt =Convert.ToDateTime(txtduedate.Text);
                    result = result.Where(o => o.ModifiedDate >= dt).ToList();
                }
        ...
        public void BindUsers()
        {
            try
            {
                ...
                ddlSearchUsers.DataSource = ulist;
                ddlSearchUsers.DataTextField = "ContractorName";
                ddlSearchUsers.DataValueField = "ID";
                ddlSearchUsers.DataBind();
                ddlSearchUsers.Items.Insert(0, new ListItem("Assign To", "0"));

                if(sessionKeys.pageid >0)
                {
                    ddlSearchUsers.SelectedValue = sessionKeys.UID.ToString();
        ...
        protected void ddlSearchUsers_SelectedIndexChanged(object sender, EventArgs e)
        {
            BindListView();
        }

        protected void btntaskserach_Click(object sender, EventArgs e)
        {
            BindListView();
        }
    }
