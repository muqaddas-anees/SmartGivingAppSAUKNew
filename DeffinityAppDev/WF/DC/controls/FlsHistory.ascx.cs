...
public partial class DC_controls_FlsHistory : System.Web.UI.UserControl
{
    public int CallID
    {
        get;
    ...
    public void DisplayHistory(int cid)
    {
        try
        {
            ...
            List<History> hList = ws.BindFLSHistory(cid);
            //callid = cid;
        ...
        try
        {
            if ((e.Item.ItemType != ListItemType.Header) && (e.Item.ItemType != ListItemType.Footer))
            {
                ...
                foreach (History h in hList)
                {
                    ...
                    lblfieldname.Text = string.Format("{0}<br />", h.FieldName.ToString());
                    lblfieldvalue.Text = string.Format("{0}<br />", h.FieldValue.ToString());
                    lblmby.Text = h.ModifiedBy.ToString();
    ...
    protected void chk_visibility_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            dw.Fls_CustomerVisiblityUpdate(int.Parse(hid.Value), Convert.ToDateTime(h_date.Value), cb.Checked);
