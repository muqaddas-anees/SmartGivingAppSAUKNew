...
public partial class DC_controls_AccessControlHistory : System.Web.UI.UserControl
{
    ...
    public void DisplayHistory(int cid)
     {
        try
        {
            ...
            List<History> hList = ws.BindAccessControlHistory(cid);
            //calid = cid;
    ...
    protected void dlstHistory_ItemDataBound(object sender, DataListItemEventArgs e)
    {
        try
        {
            if ((e.Item.ItemType != ListItemType.Header) && (e.Item.ItemType != ListItemType.Footer))
            {
                ...
                Label lblvname = (Label)e.Item.FindControl("lblvname");
                ...
                foreach (History h in hList)
                {
                    ...
                    if (int.Parse(hid.Value) != 0)
                        clid = int.Parse(hid.Value);
                    else
                        clid = int.Parse(hid.Value);
                    CallDetailsJournal cdj = CallDetailsJournalBAL.SelectByDate(clid);
                    ...
                    if (h.VisitorName != null)
                    {
                        lblvname.Text = h.VisitorName.ToString();
                        trdata.Visible = true;
                    }
                    else
                    {
    ...
    protected void chk_visibility_CheckedChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            dw.AccessControl_CustomerVisiblityUpdate(int.Parse(hid.Value), Convert.ToDateTime(h_date.Value), cb.Checked);
