...
namespace DeffinityAppDev.WF
{
    public partial class ForgotPassword : System.Web.UI.Page
    {
        ...
        public void PasswrdSendToMail(UserMgt.Entity.v_contractor con, string PwdLink)
        {
            try
            {
                var pdata = PortfolioMgt.BAL.PartnerDetailBAL.PartnerDetailBAL_SelectAll().Where(o => o.ID == con.PartnerID).FirstOrDefault();

                Email em = new Email();
                string Body = File.ReadAllText(System.Web.HttpContext.Current.Server.MapPath("~/WF/DC/EmailTemplates/PasswordSendingmail.html"));
                Body = Body.Replace("[Logo]", pdata.ParnerPortal + System.Configuration.ConfigurationManager.AppSettings["maillogo"]);
                ...
                em.SendingMail(pdata.FromEmail, con.ContractorName + " Password Reset", Body, con.EmailAddress, "");
            }
            catch(Exception ex)
            {
        ...
        private void Username()
        {
            ...
            using (UserDataContext ud = new UserDataContext())
            {
                ...
                lc = (from a in ud.v_contractors where a.LoginName.ToLower() == txtEmail.Text.ToLower().Trim() && a.PartnerID == sessionKeys.PartnerID select a).FirstOrDefault();
                if (c != null)
                {
            ...
            try
            {
                if (txtEmail.Text.Trim() != "")
                {
                    using (UserDataContext ud = new UserDataContext())
                    {
                        ...
                        Fpwd = new ForgotPasswordInfo();
                        var lc = new UserMgt.Entity.v_contractor();
                        lc = (from a in ud.v_contractors where a.LoginName == txtEmail.Text.Trim() && a.PartnerID == sessionKeys.PartnerID select a).FirstOrDefault();
                        if (lc != null)
                        {
                            UserMgt.Entity.ForgotPasswordInfo ForgotInfo = (from a in ud.ForgotPasswordInfos
