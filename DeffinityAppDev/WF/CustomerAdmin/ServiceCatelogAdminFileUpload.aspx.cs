...
public partial class ServiceCatelogAdminFileUpload : System.Web.UI.Page
{
    ...
    public void FillData(DataSet ds, int PortfoliioID)
    {
        foreach (DataTable tbl in ds.Tables)
        {
            ...
            if (tbl.Rows.Count > 0)
            {
                ...
                string dataColumn_category = string.Empty;
                string dataColumn_subCategory = string.Empty;
                string dataColumn_markup = string.Empty;
                string dataColumn_desc = string.Empty;
                string dataColumn_part = string.Empty;
                string dataColumn_nsp = string.Empty;
                string dataColumn_date = string.Empty;
                string dataColumn_discountprice = string.Empty;

                foreach (DataColumn column in tblData.Columns)
                {
                    if (column.ColumnName.Trim().ToString().ToUpper().Equals("CATEGORY"))
                        dataColumn_category = column.ColumnName.ToString();
                    if (column.ColumnName.Trim().ToString().ToUpper().Equals("SUB-CATEGORY"))
                        dataColumn_subCategory = column.ColumnName.ToString();
                    if (column.ColumnName.ToString().ToUpper().Contains("PART"))
                        dataColumn_part = column.ColumnName.ToString();
                    if (column.ColumnName.ToString().ToUpper().Contains("DESCRIPTION"))
                        dataColumn_desc = column.ColumnName.ToString();
                    if (column.ColumnName.ToString().ToUpper().Contains("NSP"))
                        dataColumn_nsp = column.ColumnName.ToString();
                    if (column.ColumnName.ToString().ToUpper().Contains("% MRK UP") || column.ColumnName.ToString().ToUpper().Contains("MARK UP"))
                        dataColumn_markup = column.ColumnName.ToString();
                    ...
                    if (column.ColumnName.ToString().ToUpper().Contains("DATE"))
                        dataColumn_date = column.ColumnName.ToString();
                    if (column.ColumnName.ToString().ToUpper().Contains("DISCOUNT"))
                        dataColumn_discountprice = column.ColumnName.ToString();
                }

                foreach (DataRow rwUniq in tblData.Rows)
                {
                    if (rwUniq[dataColumn_category].ToString() != "")
                    {
                        string categoryName = rwUniq[dataColumn_category].ToString();
                        string subCategoryName = rwUniq[dataColumn_subCategory].ToString();
                        string description = rwUniq[dataColumn_desc].ToString();
                        string partNumber = rwUniq[dataColumn_part].ToString();
                        ...
                        if (rwUniq[dataColumn_nsp] != null)
                        {
                            if (rwUniq[dataColumn_nsp].ToString() != "")
                            {
                                NSPPrice = rwUniq[dataColumn_nsp].ToString();
                                NSPPrice = rwUniq[dataColumn_nsp].ToString().Replace('£', ' ').Trim();
                        ...
                        if (rwUniq[dataColumn_markup] != null)
                        {
                            if (rwUniq[dataColumn_markup].ToString() != "")
                            {

                                MRKUP = rwUniq[dataColumn_markup].ToString();
                                //MRKUP = MRKUP * 100;
                               // MRKUP = MRKUP;
                            }
                            else
                        ...
                        //if (rwUniq[dataColumn_date] != null)
                        //{
                        //    if (rwUniq[dataColumn_date].ToString() != "")
                        //    {
                        //        dateadded = rwUniq[dataColumn_date].ToString();
                        //    }
                        ...
                        if (rwUniq[dataColumn_discountprice] != null)
                        {
                            if (rwUniq[dataColumn_discountprice].ToString() != "")
                            {
                                discountPrice = rwUniq[dataColumn_discountprice].ToString();
                            }
                            else
                            {
                                discountPrice = "0.00";
                        ...
                        CatalogueInsert(categoryName, subCategoryName, vendorID, description, partNumber, buyingPirce, sellingPrice, NSPPrice, unit, MRKUP.ToString(),dateadded, PortfoliioID, discountPrice);
                    }
                }
            }
        }
    ...
    private int CatalogueInsert(string CategoryName, string SubCategoryName, int VendorID, string Description, string PartNumber, string BuyingPrice, string SellingPrice, string NSPPrice, string Unit, string MRKUP,string datemodified,int PortfolioID,string discountprice)
    {
        int OutID = 0;
        try
        {
            ...
            using (PortfolioDataContext pd = new PortfolioDataContext())
            {
                ...
                if (ss == null)
                {
                    ...
                    //ss.BuyingPrice = 0;
                    ss.DiscountPrice = Convert.ToDouble(string.IsNullOrEmpty(discountprice) ? "0.00" : discountprice);
