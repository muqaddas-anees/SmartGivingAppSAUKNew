...
{
    public partial class ContactAddressDetailsBasic : System.Web.UI.Page
    {
                ...
                if (!IsPostBack)
                {
                   ...
                   // txtAddons.Text = "0";
                    ...
                    if (Request.QueryString["ContactID"] != null)
                    {
                        ...
                        BindContactDetails();
                        BindKeyContact(Convert.ToInt32(Request.QueryString["ContactID"].ToString()));
                    ...
                    if (Request.QueryString["addid"] != null)
                    {
                        ...
                        SetKeyContact(Convert.ToInt32(Request.QueryString["addid"]));
                    }
        ...
        private void BindKeyContact(int ContactID)
        {
            try
            {
                
                var kList = PortfolioMgt.BAL.CustomerKeyContactsBAL.CustomerKeyContact_SelectAll(ContactID).OrderBy(o=>o.Name).ToList();
                ddlKeyContact.DataSource = kList;
                ddlKeyContact.DataTextField = "Name";
                ddlKeyContact.DataValueField = "ID";
                ddlKeyContact.DataBind();
                ddlKeyContact.Items.Insert(0, new ListItem("Please select...", "0"));
            }
            catch(Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
        ...
        private void UpdateKeyContact(int AddressID)
        {
            try
            {
                if (Convert.ToInt32(ddlKeyContact.SelectedValue) > 0)
                {
                    PortfolioMgt.BAL.CustomerKeyContactsBAL.CustomerKeyContact_AssociateAddress_addUpdate(Convert.ToInt32(ddlKeyContact.SelectedValue), AddressID);
                }
            }
            catch(Exception ex)
            {
        ...
        private void SetKeyContact(int AddressID)
        {
            try
            {

                ddlKeyContact.SelectedValue = PortfolioMgt.BAL.CustomerKeyContactsBAL.CustomerKeyContact_AssociateAddress_select(AddressID).ToString();
            }
            catch (Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
