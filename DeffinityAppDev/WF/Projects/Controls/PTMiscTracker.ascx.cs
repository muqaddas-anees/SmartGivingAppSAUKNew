...
public partial class controls_PTMiscTracker : System.Web.UI.UserControl
{
    ...
    private void BindGrid()
    {
        try
        {
            ...
            {
                var miscList = (from r in db.ProjectBOMDetils
                                    join b in db.BOM_Types on r.WorkSheetID equals b.ID
                                where (b.IsDeleted.HasValue ? b.IsDeleted.Value : false) == false && r.ProjectReference == QueryStringValues.Project && r.Mics != 0 && r.ID != -99
                                    select new
                                    {
                                        ID = r.ID,
                                        r.Worksheet,
                                        ...
                                        Total = r.Qty * r.Mics, // BoM Quantities * Unit cost
                                        OrderToDate = (r.Mics * (r.QtyReceived.HasValue ? r.QtyReceived : 0)),
                ...
                {
                    miscList = miscList.Where(l => l.WorkSheetID == Convert.ToInt32(ddlWorksheet.SelectedValue)).ToList();
                }
                if (!string.IsNullOrEmpty(txtDescription.Text.Trim()))
                {
                    miscList = miscList.Where(l => l.Description.ToLower().Contains(txtDescription.Text.ToLower())).ToList();
                }
                total = miscList.Select(m => m.Total).Sum();
                spentToDate = Convert.ToDouble(miscList.Select(m => m.OrderToDate).Sum());
                totalCostRemaining = total - spentToDate;


                gvMaterial.DataSource = miscList;
                gvMaterial.DataBind();
                        ...
                        if (inventoryId == "0")
                        {
                            ...
                            else if (im != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    var stockList = db.Inventory_SiteStorageDetails.Where(s => s.InventoryId == im.Id && s.UseStatus == false).Take(int.Parse(qty)).ToList();
