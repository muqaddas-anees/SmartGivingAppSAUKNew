...
public partial class controls_Documents : System.Web.UI.UserControl
{
    ...
    protected void CustomerPortalProjectDocuments_NewTreeBinding()
   {
        ...
        cmdSQL1.Parameters.AddWithValue("@CurrentUserID", sessionKeys.UID);
        //}
    ...
    protected void CustomerAndPortalAdmin_NewTreeBinding()
    {
        ...
        cmdSQL1.Parameters.AddWithValue("@PortfolioID", sessionKeys.PortfolioID);
        //}
        cmdSQL1.Connection = cn;
        SqlDataAdapter da11 = new SqlDataAdapter(cmdSQL1);
        da11.Fill(ds11, "MasterDoc");
    ...
    private void NewTreeBinding()
    {
        ...
        rootNode.DataKey = 0;
        rootNode.Text = "<i class='fa fa-folder'></i> " + Resources.DeffinityRes.FilesFolders;// "Files/Folders";
    ...
    protected void linkFolder_Click(object sender, EventArgs e)
    {
        ...
        {
            Session["update"] = Server.UrlEncode(System.DateTime.Now.ToString());
            LinkButton linkFolder = (LinkButton)sender;
    ...
    //    }
    //}
    protected void btnCloseCheckin_click(object sender, EventArgs e)
    {
        modelChekin.Hide();
    }
    protected void ImageFile_Click(object sender, EventArgs e)
    {
        if (Session["update"].ToString() == ViewState["update"].ToString())
        {
            ...
            {
                string[] t = linkFile.CommandArgument.Split(',');
                // 0 is document id
                CheckInOutUpdate(true, t[0]);
                iframeCheckin.Attributes.Add("src", string.Format("../WF/Projects/POPCheckin.aspx?ID={0}&folder={1}&mode={2}", t[0], t[1], t[2].ToLower()));
                //iframeCheckin.Load();
                if(t[2].ToLower() =="true")
                modelChekin.Show();
            }
            else if (linkFile.ToolTip.ToLower().Contains("check in"))
            {
                string[] t = linkFile.CommandArgument.Split(',');
                // 0 is document id
                CheckInOutUpdate(false, t[0]);
                iframeCheckin.Attributes.Add("src", string.Format("../WF/Projects/POPCheckin.aspx?ID={0}&folder={1}&mode={2}", t[0], t[1], t[2]));
                //iframeCheckin.Load();
                modelChekin.Show();
    ...
    protected void ImageFile1_Click(object sender, EventArgs e)
    {
        if (Session["update"].ToString() == ViewState["update"].ToString())
        {
            Session["update"] = Server.UrlEncode(System.DateTime.Now.ToString());
    ...
    protected void UltraWebTree1_NodeClicked(object sender, Infragistics.WebUI.UltraWebNavigator.WebTreeNodeEventArgs e)
    {
        ...
        {
            if (Request.QueryString["mode"] == null)
                Response.Redirect(string.Format("{0}?project={1}&folderID={2}", Request.Url.AbsolutePath, projectID, folderID));
            else
                Response.Redirect(string.Format("{0}?mode=central&folderID={2}", Request.Url.AbsolutePath, ddlProject.SelectedValue, folderID));
    ...
    {
        lblMsg.Text = string.Empty;
        if(e.CommandName.Equals("checkinPopup"))
        {
    ...
    protected bool ShowHide()
    {
        ...
        if ((Request.Url.ToString().ToLower()).Contains("customerdocs.aspx") == true)
        {
            del = false;
        }
        return del;
    ...
    protected void SDdocuments_Load()
    {
        ...
        if (Request.QueryString["folderID"] == null)
            ...
            folderID = Convert.ToInt32(getFolderID());
    ...
    {
        string tmp = HttpUtility.HtmlDecode(Request.QueryString["folderID"].ToString());
        int strIndex = tmp.IndexOf("?");
        if (strIndex > 0)
            tmp = tmp.Substring(0, strIndex);
        return tmp;
    }
    ...
    protected void CustomerAndPortalAdmin_Load()
    {
        if (sessionKeys.SID <= 2)
        {
            ...
            //LABEL2.Visible = true;
            gridFiles.Columns[9].Visible = false;
            //PnlAllowCustomers.Visible = true;
        }
        else
        {
            //IMAGE1.Visible = false;
    ...
    protected void healthdocuments_PreRender()
    {
        ...
        if (Request.QueryString["HealthCheckId"] != null)
        {
            ...
            strUrl = "../../WF/Health/healthcheckformdocs.aspx?HealthCheckId=" + strHealthCheckId + "&PID=" + strPID;
        }
    ...
    protected void RFIdocuments_PreRender()
    {
        ...
        if (Request.QueryString["project"] != null)
        {
            ...
            strUrl = "../../WF/Vendor//RFIDocuments.aspx.aspx?project=" + strPojectID;
        }
        else
        {
            //strUrl = "/Deffinity_team/ProjectDocuments.aspx?mode=central";
            strUrl = "../../WF/Vendor/RFIDocuments.aspx?mode=central";
        }
  ...
  //          if (lblMsg.Text.Trim() != "Folder deleted successfully.")
    ...
    protected void CustomerProjectdocuments_PreRender()
    {
        ...
        if (Request.QueryString["project"] != null)
        {
            strPojectID = Request.QueryString["project"].ToString();
            //strUrl = "/Deffinity_team/ProjectDocuments.aspx?project=" + strPojectID;
            strUrl = "../../WF/Projects/CustomerProjectDocuments.aspx?project=" + strPojectID;
        }
        else
        {
            //strUrl = "/Deffinity_team/ProjectDocuments.aspx?mode=central";
            strUrl = "../../WF/Projects/CustomerProjectDocuments.aspx?mode=central";
        }
        ...
        if (sessionKeys.SID > 3 && Request.QueryString["project"] != null)
        {
            foreach (GridViewRow row in gridFiles.Rows)
            {
                ...
                if (isRestricted)
                {
                    ...
                    ((LinkButton)row.Cells[0].FindControl("lnkDownLoad")).Enabled = false;
                    ((LinkButton)row.Cells[0].FindControl("lnkDownLoad")).ForeColor = System.Drawing.Color.Gray;
                    ((LinkButton)row.Cells[row.Cells.Count - 1].FindControl("btnDelete")).Enabled = false;
                }
            }
        }
        //if (sessionKeys.SID <= 3)
    ...
    protected void Projectdocuments_PreRender()
    {
        ...
        if (Request.QueryString["project"] != null)
        {
            strPojectID = Request.QueryString["project"].ToString();
            //strUrl = "/Deffinity_team/ProjectDocuments.aspx?project=" + strPojectID;
            strUrl = "../../WF/Projects/ProjectDocuments.aspx?project=" + strPojectID;
        }
        else
        {
            //strUrl = "/Deffinity_team/ProjectDocuments.aspx?mode=central";
            strUrl = "../../WF/Projects/ProjectDocuments.aspx?mode=central";
        }
    ...
    protected void MyTaskdocuments_PreRender()
    {
        ...
        if (Request.QueryString["project"] != null)
        {
            ...
            strUrl = "../../WF/Resource/MyTasksDocuments.aspx?project=" + strPojectID;
        }
        else
        {
            //strUrl = "/Deffinity_team/MyTasksDocuments.aspx?mode=central";
            strUrl = "../../WF/Resource/MyTasksDocuments.aspx?mode=central";
        }
    ...
    protected string PermissionURL(int Id)
    {
        if (Request.QueryString["project"] != null)
        {
            string url = string.Format("../../WF/Projects/DocumentPermission.aspx?DocumentID={0}&project={1}", Id, Request.QueryString["project"].ToString());
            return url;
        }
        return string.Empty;
    }
    ...
    protected string ResetEnableIcon(string ContractorID, string CheckOut)
    {
        ...
        if (ContractorID.Trim() == sessionKeys.UID.ToString())
        {
            if (CheckOut.Trim() == "True")
            {
                ...
                imageURL = "../../WF/images/ico_reset.gif";
            }
            else
            {
                //imageURL = "/Deffinity_team/images/ico_reset_grey.gif";
                imageURL = "../../WF/images/ico_reset_grey.gif";                
            }            
        }
        else
        {
    ...
    {
        string imageURL = string.Empty;
        imageURL = "../../WF/media/ico_unchecked.png";
        if (CheckOut.Trim() == "1")
        {

            imageURL = "../../WF/media/ico_checked.png";
    ...
    protected string GetCheckInOutIcon(string CheckOut)
    {
        ...
        if (CheckOut.Trim() == "False")
        {
            ...
            imageURL = "~/WF/media/ico_checkout.gif";
        }
        else
        {
             
             //imageURL = "/Deffinity_team/media/ico_checkin.gif";
             imageURL = "~/WF/media/ico_checkin.gif";
    ...
    protected string GetIcon(string fileName)
    {
        ...
        switch (fileExtension.ToLower())
        {
            case "xls":
            case "xlsx":
                imageURL = "<i class='fa fa-file-excel-o'></i>";
                break;
            case "doc":
            case "docx":
                imageURL = "<i class='fa fa-file-word-o'></i>";
                break;
            case "jpeg":
            case "jpg":
            case "png":
            ...
            case "dxf":
            case "3dm":
                imageURL = "<i class='fa fa-file-picture-o'></i>";
                break;
            case "aac":
            case "aif":
            case "iff":
            ...
            case "vob":
            case "wmv":
                imageURL = "<i class='fa fa-file-video-o'></i>";
                break;
            case "7z":
            case "deb":
            case "gz":
            ...
            case "zip":
            case "zipx":
                imageURL = "<i class='fa fa-file-zip-o'></i>";
                break;
            case "txt":
                imageURL = "<i class='fa fa-file-text-o'></i>";
                break;
            case "pdf":
                imageURL = "<i class='fa fa-file-pdf-o'></i>";
                break;
            case "ppt":
            case "pptx":
                imageURL = "<i class='fa fa-file-powerpoint-o'></i>";
                break;
            case "vsd":
            case "vsdx":
                imageURL = "<i class='fa fa-file-text-o'></i>";
                break;
            default:
                imageURL = "<i class='fa fa-file-o'></i>";
                break;
        }
        return imageURL;
    }
    ...
    private void DisplayResults(SearchResultList searchResults)
    {
        ...
        if (searchResults.Count <= 0)
            ...
            foreach (SearchResult result in searchResults)
            {
                ...
                sb.Append(string.Format("<strong><a style='color:blue;padding-left:5px;text-decoration:underline;' target='_blank' href='../../WF/Projects/Download.aspx?FileID={1}'>{0}</a></strong></span><br/>", result.fileName, result.fileID));
    ...
    protected void AjaxFileUpload1_OnUploadComplete(object sender, AjaxFileUploadEventArgs file)
    {
        try
        {
            ...
            var fileContentLength = file.FileSize;
            byte[] imageBytes = file.GetContents();
            var filecontent = imageBytes;

            AC2P_DocumentsController AC2PDocumentsController = new AC2P_DocumentsController();
            AC2PDocumentsController.DN_ProjectUploadInsertNew(QueryStringValues.Project.ToString(), fileName, filecontent, fileName, fileContentType, "P", fileContentLength, int.Parse(folderID), sessionKeys.UID, sessionKeys.IncidentID);
    ...
    {
        NewTreeBinding(); 
        //string folderID = getFolderID().ToString();
        //string project = Request.QueryString["project"].ToString();
        //Response.Redirect("ProjectDocuments.aspx?project=" + project + "&folderID=" + folderID);
    }
