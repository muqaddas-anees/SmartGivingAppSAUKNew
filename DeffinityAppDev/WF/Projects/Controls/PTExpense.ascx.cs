...
public partial class controls_PTExpense : System.Web.UI.UserControl
{
                ...
                using (UserDataContext ud = new UserDataContext())
                {
                    ...
                    var result = (from e in expenseList
                                  ...
                                  select new ExpenseList
                                  {
                                      ...
                                      Total = (e.Qty.HasValue ? e.Qty : 0) * (e.UnitCost.HasValue ? e.UnitCost : 0), // Qty * UnitCost
    ...
    public int InsertExternalExpenses(int projectReference, int contractorId, int entryTypeId, DateTime date, double amount, string notes, string description, double qty, double unitCost, int assignedTo, bool expensed)
    {
        try
        {
    ...
    public void UpdateExternalExpenses(int id, int projectReference, int contractorId, int entryType, DateTime date, double amount, string notes, string description, double qty, double unitCost, int assignedTo, bool expensed)
    {
    ...
    protected void gvExpense_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "Insert_ExternalFooter")
            {
                ...
                double unitCost = Convert.ToDouble(((TextBox)gvExpense.FooterRow.FindControl("txtFooterUnitCost")).Text.Trim());
                ...
                InsertExternalExpenses(Convert.ToInt32(QueryStringValues.Project), Convert.ToInt32(Session["UID"].ToString()),
                     entryType, date, amount, "", description, qty, unitCost, assignedTo, expensed);
                BindGrid();
            ...
            if (e.CommandName == "Update")
            {
                ...
                double unitCost = Convert.ToDouble(((TextBox)Row.FindControl("txtUnitCost")).Text.Trim());
                ...
                UpdateExternalExpenses(id, Convert.ToInt32(QueryStringValues.Project), Convert.ToInt32(Session["UID"].ToString()), entryType, date, amount, "", description, qty, unitCost, assignedTo, expensed);
