    ...
    public partial class InventoryNew : System.Web.UI.UserControl
    {
        ...
        private void BindProductsSection(string id)
        {
            try
            {
                ...
                if (_dt.Rows.Count > 0)
                {
                    ...
                    String tempDate = FormateDate(tDate.ToString("dd/MM/yyyy"));
                    ...
                    String tempOrdDate = FormateDate(OrdDate.ToString("dd/MM/yyyy"));
                    ...
                    using (UserDataContext ud = new UserDataContext())
                    {
                        ...
                        if (typeOfExport == "Excel")
                        {
                            ...
                            foreach (var item in usageReport)
                            {
                                ...
                                ws.Cell("A" + x.ToString()).Style.DateFormat.Format = "dd/MM/yyyy";
                            ...
                            foreach (var item in summaryReport)
                            {
                                ...
                                ws2.Cell("A" + y.ToString()).Style.DateFormat.Format = "dd/MM/yyyy";
                                ws2.Cell("B" + y.ToString()).Value = item.EndDate;
                                ws2.Cell("B" + y.ToString()).Style.DateFormat.Format = "dd/MM/yyyy";
        ...
        protected void grdInventory_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    gv = (GridView)row.FindControl("InnerGridHistory");
                    //if (gv.UniqueID == gvUniqueID)
                    //{
                    //    gv.PageIndex = gvNewPageIndex;
                    //    gv.EditIndex = gvEditIndex;
                    //}
                    BindHistoryInnerGrid(int.Parse(imgDelete.CommandArgument.ToString()), gv);

                    //GridView gvSites = (GridView)row.FindControl("GridSites");
                    //BindRelatedSitesGrid(gvSites, int.Parse(imgDelete.CommandArgument.ToString()));
        ...
        private void InventoryGridBind(bool isInserted = false)
        {
            try
            {
                ...
                {
                    grdInventory.Columns[grdInventory.Columns.Count - 1].Visible = true;
                    grdInventory.Columns[grdInventory.Columns.Count - 2].HeaderStyle.CssClass = "";
                }
                else
                {
                ...
                {
                    grdInventory.Columns[0].Visible = false;
                    grdInventory.Columns[1].HeaderStyle.CssClass = "header_bg_l";
                }
                //batch option
                grdInventory.Columns[13].Visible = Convert.ToBoolean(hBatchoption.Value == string.Empty ? "True" : hBatchoption.Value);
            }
            catch (Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
        ...
        protected void btnSubmitInPopUp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (BatchControlRblist.SelectedItem.Text == "Yes")
                    {
                        Inventory_Batch_Add_InAddStock(inventoryid, int.Parse(txtQuantity.Text), true);
                    }
                    else
                    {
                        Inventory_Batch_Add_InAddStock(inventoryid, int.Parse(txtQuantity.Text), false);
                    }
                }
        ...
        public void Inventory_Batch_Add_InAddStock(int inventoryID, int Qty, bool BatchControl)
        {
                    ...
                    using (InventoryDataContext db = new InventoryDataContext())
                    {
                            ...
                            Convert.ToInt32(ddlSubcategoryInNewItem.SelectedValue), 0, Convert.ToInt32(ddlsiteInNewItem.SelectedValue)) == false)
                        {
                            ...
                            if (inventoryID > 0)
                            {
                                ...
                                {
                                    InsertIntoNewStockOnItemCreation(int.Parse(txtQtyNewItem.Text), inventoryID, true);
                                    //Batch reference
                                    ib = Inventory_Batch_Add(inventoryID, Convert.ToInt32(txtQtyNewItem.Text.Trim()), true);
                                }
                                else if (selectedText.ToLower() == "no")
                                {
                                    InsertIntoNewStockOnItemCreation(int.Parse(txtQtyNewItem.Text), inventoryID, false);
                                    //Batch reference
        ...
        public void InsertIntoNewStockOnItemCreation(int Qty, int ProductId, bool BatchControl)
        {
            try
            {
                INS = new InventoryRepository<Inventory_New_Stock>();
        ...
        public void BindFilteredBatchGrid(string Productid)
        {
            try
            {
                ...
                foreach (var d in bList)
                {
                    ...
                    {
                        datarw[0] = d.ID;
                        datarw[1] = string.Format("{0:dd/MM/yyyy}", d.DateReceived.HasValue ? d.DateReceived.Value : Convert.ToDateTime("01/01/1900"));
        ...
        protected void btnViewAll_Click(object sender, EventArgs e)
        {
            try
            {
                grdInventory.AllowPaging = false;
                InventoryGridBind();
            }
            catch (Exception ex)
            {
