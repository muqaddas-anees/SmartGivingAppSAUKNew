...
namespace DeffinityAppDev.WF.Inventory.Controls
{
    public partial class InventoryNew : System.Web.UI.UserControl
    {
        ...
        InventoryRepository<Inventory_Batch> In_batch = null;
        ...
        Inventory_BatchCustomData In_IBCD = null;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                BindBatchPlaceholderFieldsInAddStock();
                //BindBatchPlaceholderFields();
                //}

                //Default batch checking
        ...
        public int GetUseOfInventory(string Productid)
        {
            int sumvalue = 0;
            try
            {
                var ibatchRepository = new InventoryRepository<Inventory_Batch>();
                var bList = ibatchRepository.GetAll().Where(o => o.InventoryID == int.Parse(Productid) && o.BatchControl == false).FirstOrDefault();
                ...
                PSDList = IMPSD.GetAll().Where(p => p.ProductId == int.Parse(Productid)).ToList();
                if (bList != null)
                {
                    PSDList = PSDList.Where(a => a.BatchID != bList.ID).ToList();
                }
                if (PSDList != null)
                {
                    sumvalue = PSDList.Select(o => o.QtyUsed.Value).Sum();
                }
            }
            catch (Exception ex)
            {
        ...
        public int GetAvailable_Qty_In_batchs(string Productid)
        {
            int Sumvalue = 0;
            try
            {
                InventoryRepository<InventoryManager_PSDProduct> IMPSD = new InventoryRepository<InventoryManager_PSDProduct>();
                var ibatchRepository = new InventoryRepository<Inventory_Batch>();
                var bList = ibatchRepository.GetAll().Where(o => o.InventoryID == int.Parse(Productid) && o.BatchControl == true).Select(a => a.QTY).Sum();
                if (bList != null)
                {
                    Sumvalue = bList.Value - GetUseOfInventory(Productid);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            System.IO.FileInfo fileInfo = new System.IO.FileInfo(path + "\\" + "SummaryReport.xlsx");
            if (fileInfo.Exists)
            {
        ...
        public void ExportDataToExcelFile(string Type)
        {
            try
            {
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    using (UserDataContext ud = new UserDataContext())
                    {
                        ...
                        string fileName = sessionKeys.PortfolioName + "_SummaryReport_" + DateTime.Now;
                        ...
                        var FinalResultList = (from a in inventoryList
                                               where a.PortfolioID == sessionKeys.PortfolioID
                                               select new
                                               {
                                                   CategoryName = categoryList.Where(c => c.ID == a.Categoery).Select(c => c.CategoryName).FirstOrDefault(),
                                                   SubCategoryName = categoryList.Where(c => c.ID == a.Categoery).Select(c => c.CategoryName).FirstOrDefault(),
                                                   Description = a.ItemDescription,
                                                   Manufacturer = a.Manufacturer.Value > 0 ? ManufacturerList.Where(m => m.Id == a.Manufacturer).Select(m => m.Name).FirstOrDefault() : "",
                                                   Site = siteList.Where(s => s.ID == a.SiteId).Select(s => s.Site1).FirstOrDefault(),
                                                   TotalQuantity = getActualStackLaevelInMainGrid(a.Id.ToString()),
                                                   QuantityDeployed = GetUseOfInventory(a.Id.ToString()),
                                                   QuantityinBatchControl = GetAvailable_Qty_In_batchs(a.Id.ToString()),
                                                   QuantityinFreeStock = GetAvailable_Qty_In_Default_batch(a.Id.ToString())
                                               }).Reverse().ToList();
                        ...
                        if (Type == "Excel")
                        {
                            ...
                            ws.Cell("B1").Value = sessionKeys.PortfolioName;

                            ws.Cell("A2").Value = "Summary Report";

                            ws.Cell("A3").Value = "Category";
                            ws.Cell("B3").Value = "Sub-Category";
                            ws.Cell("C3").Value = "Description";
                            ws.Cell("D3").Value = "Manufacturer";
                            ws.Cell("E3").Value = "Site";
                            ws.Cell("F3").Value = "Total Quantity";
                            ws.Cell("G3").Value = "Quantity Deployed";
                            ws.Cell("H3").Value = "Quantity in Batch Control";
                            ws.Cell("I3").Value = "Quantity in Free Stock";
                            ...
                            {
                                ws.Cell("A" + x.ToString()).Value = item.CategoryName;
                                ws.Cell("B" + x.ToString()).Value = item.SubCategoryName;
                                ws.Cell("C" + x.ToString()).Value = item.Description;
                                ws.Cell("D" + x.ToString()).Value = item.Manufacturer;
                                ws.Cell("E" + x.ToString()).Value = item.Site;
                                //ws.Cell("E" + x.ToString()).Style.Font.Bold = true;
                                ws.Cell("F" + x.ToString()).Value = item.TotalQuantity;
                                ws.Cell("G" + x.ToString()).Value = item.QuantityDeployed;
                                ws.Cell("H" + x.ToString()).Value = item.QuantityinBatchControl;
                                ws.Cell("I" + x.ToString()).Value = item.QuantityinFreeStock;
                                x++;
                            }

                            // From worksheet
                            var rngTable = ws.Range("A1:I4");

                            var rngHeaders = rngTable.Range("A3:I3");
                            ...
                            string path = HttpContext.Current.Server.MapPath("~/WF/UploadData/Inventory/SummaryReport");
                            ...
                            wb.SaveAs(path + "\\" + "SummaryReport.xlsx");
        ...
        public void BindCustomerInSearch()
        {
            try
            {
                ...
                ddlcategoryInSearch.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select...", "0"));
                ddlsubcategoryInSearch.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select...", "0"));
                if (sessionKeys.PortfolioID > 0)
                {
                    ddlcustomerInSearch.SelectedValue = sessionKeys.PortfolioID.ToString();
                    BindSiteInSearch();
                    BindCategory_InSearch(sessionKeys.PortfolioID);
                    BindProductInSearch();
                }
               // InventoryGridBind();
                BindData_SearchUsageGrid();
        ...
        public void BindSiteInNewItem()
        {
            try
            {
                ...
                ddlsiteInNewItem.DataSource = Deffinity.Bindings.DefaultDatabind.b_SiteSelect_Portfilio(int.Parse(ddlcustomerInNewItem.SelectedValue));
        ...
        public void BindProductInSearch()
        {
            try 
            {
                IMCS = new InventoryManagerCs();
                int categoryId = 0;
                if ((ddlcategoryInSearch.SelectedItem.Value != null))
                {
                    categoryId = Convert.ToInt32(ddlcategoryInSearch.SelectedItem.Value.ToString());


                    int subcategoryId = 0;
                    if ((ddlsubcategoryInSearch.SelectedItem.Value != null))
                    {
                        subcategoryId = Convert.ToInt32(ddlsubcategoryInSearch.SelectedItem.Value.ToString());
                    }
                    int siteId = 0;
                    if ((ddlsiteInSearch.SelectedItem.Value != null))
                    {
                        siteId = Convert.ToInt32(ddlsiteInSearch.SelectedItem.Value.ToString());
                    }

                    var result = IMCS.SelectProducts(sessionKeys.PortfolioID, categoryId,
                    ...
                    ddlProductInSearch.DataSource = result;
                    ddlProductInSearch.DataValueField = "Id";
                    ddlProductInSearch.DataTextField = "ItemDescription";
                    ddlProductInSearch.DataBind();
                    ddlProductInSearch.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select...", "0"));
                }
                else
                {
                    ddlProductInSearch.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please Select...", "0"));
        ...
        private void BindCategory_InSearch(int Portfolio)
        {
            ddlcategoryInSearch.Items.Clear();
            SC_cntMaterialCS = new SC_ContractorMaterial();
            ddlcategoryInSearch.DataSource = SC_cntMaterialCS.SelectAllCategory(Portfolio, PageType, VendorID);
            ddlcategoryInSearch.DataValueField = "ID";
            ddlcategoryInSearch.DataTextField = "CategoryName";
            ddlcategoryInSearch.DataBind();
            //ddlcategoryInSearch.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please select..","0"));

        }
        private void BindSubCategory_InSearch(int CategoryID, int PortfolioID)
        {
            ddlsubcategoryInSearch.Items.Clear();
            SC_cntMaterialCS = new SC_ContractorMaterial();
            ddlsubcategoryInSearch.DataSource = SC_cntMaterialCS.GetSubCategory(CategoryID, PortfolioID, 0);
            ddlsubcategoryInSearch.DataValueField = "Id";
            ddlsubcategoryInSearch.DataTextField = "CategoryName";
            ddlsubcategoryInSearch.DataBind();
            //ddlsubcategoryInSearch.Items.Insert(0, new System.Web.UI.WebControls.ListItem("Please select..", "0"));
        ...
        public static string GetImageUrl(Guid a_gId, ImageManager.ThumbnailSize? a_oThumbSize)
        {
            ...
            //return "~/UploadData/ThumbNails/00000000-0000-0000-0000-000000000000.png";
        }
        ...
        private void GridInventoryBindingByProduct()
        {
            ...
            grdInventoryNew.DataSource = SC_cntMaterialCS.SelectMaterial(Convert.ToInt32(ddlProduct.SelectedValue.ToString()));
            grdInventoryNew.DataBind();
        }
                ...
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    var result = (from i in db.InventoryManagers
                                  ...
                                  select new
                                  {
                                      ...
                                      Sitename = s.Site1,
                                      AreaName = string.Empty,// db.Inventory_Areas.Where(z => z.id == i.Area).Select(z => z.Name).FirstOrDefault(),
                                      LocationName = string.Empty,// db.Inventory_Locatins.Where(z1 => z1.id == i.Location).Select(z1 => z1.Name).FirstOrDefault(),
                                      ShelfName = string.Empty,// db.Inventory_Shelfs.Where(z3 => z3.id == i.Shelf).Select(z3 => z3.Name).FirstOrDefault(),
                                      BinName = string.Empty,//db.Inventory_Bins.Where(z4 => z4.id == i.Bin).Select(z4 => z4.Name).FirstOrDefault()
                                  }).ToList();
                    if (customer != 0)
                    {
                    ...
                    grdInventoryNew.DataSource = result;
                    grdInventoryNew.DataBind();
                }
            }
            catch (Exception ex)
        ...
        private void BindProductsSection(string id)
        {
            try
            {
                ...
                if (_dt.Rows.Count > 0)
                {
                    ...
                    txtStockLevel.Text = getActualStackLaevelInMainGrid(id).ToString();
        ...
        public int getActualStackLaevel(string id, int Qty, string type)
        {
            int Sumvalue = 0;
            try
            {
        ...
        public int GetAvailable_Qty_In_Default_batch(string Productid)
        {
            int Sumvalue = 0;
            try
            {
                ...
                if (bList != null)
                {
                    ...
                    PSDList = IMPSD.GetAll().Where(p => p.ProductId == int.Parse(Productid) && p.BatchID == bList.ID).ToList();
                    if (PSDList != null)
                    {
                        Sumvalue = (bList.QTY.Value - (PSDList.Select(o => o.QtyUsed.Value).Sum()));
                    }
                    else
                    {
                        Sumvalue = bList.QTY.Value;
                    }
                }
            }
            catch (Exception ex)
        ...
        public int getActualStackLaevelInMainGridWithItemDescription(string ItemDescription)
        {
            int Sumvalue = 0;
            try
            {
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    var Records = db.InventoryManagers.Where(i => i.ItemDescription == ItemDescription).OrderBy(i => i.Id).ToList();
                    foreach (var x in Records)
                    {
                        Sumvalue = Sumvalue + (int)(from a in db.InventoryManagerJournals
                                                    where a.InventoryId == x.Id && a.SectionType == "Global"
                                                    orderby a.Id descending
                                                    select a.Qty.HasValue ? a.Qty.Value : 0).FirstOrDefault();
                    }
                }
            }
            catch (Exception ex)
        ...
        public int getActualStackLaevelInMainGrid(string id)
        {
            int Sumvalue = 0;
            try
            {
        ...
        private void BindHistory(int inventoryId)
        {
            try
            {
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    using (UserDataContext ud = new UserDataContext())
                    {
                        var In_managerList = db.InventoryManagers.Where(a => a.Id == inventoryId).FirstOrDefault();
                        ...
                        var result = (from j in journalList
                                      ...
                                      where j.QtyChangedOrNot.HasValue ? j.QtyChangedOrNot == true : j.QtyChangedOrNot == null
                                      select new
                                      {
                                          ID = j.Id,
                                          ItemDescription = !string.IsNullOrEmpty(j.ItemDescription) ? j.ItemDescription : In_managerList.ItemDescription,
                                          j.Qty,
                                          j.NoDeployed,
                                          j.MdofiedDate,
                                          ...
                                          j.ReasonCode,
                                          j.TransferQty,
                                          OpeningStock = j.OpeningStock.HasValue ? j.OpeningStock.Value : j.Qty,
                                          UseCode = u.Name
                                      }).ToList();
        ...
        private void BindHistoryInnerGrid(int inventoryId, GridView Gv)
        {
            try
            {
                if (Gv != null)
        ...
        public void LastbacthRecordToBatchFields(int In_Id)
        {
            try
            {
                using (InventoryDataContext Idc = new InventoryDataContext())
                {
                    var lastbacthRecord = Idc.Inventory_Batches.Where(a => a.InventoryID == In_Id).OrderByDescending(a => a.ID).FirstOrDefault();
                    if (lastbacthRecord != null)
                    {
                        BindRecordToBatchFileds(lastbacthRecord.ID, "Clear");
                        txtBatchreference.Text = (lastbacthRecord.BatchRef.Value + 1).ToString();
                    }
                }
            }
            catch (Exception ex)
        ...
        public string GetInventoryNameWithId(int In_Id)
        {
            ...
            try
            {
                using (InventoryDataContext Idc = new InventoryDataContext())
                {
                    In_Name = Idc.InventoryManagers.Where(a => a.Id == In_Id).Select(a => a.ItemDescription).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
        ...
        protected void grdInventory_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "Batch")
                {
                    try
                    { 
                        BindPopUpGridItems(int.Parse(e.CommandArgument.ToString()));
                        BindSiteInPopup();
                        BindAreaInPopup(sessionKeys.PortfolioID);
                        lblproductName.Text = GetInventoryNameWithId(int.Parse(e.CommandArgument.ToString()));
                        LblInventoryId.Text = e.CommandArgument.ToString();

                        btnSubmitInPopUp.Visible = true;
                        BtnUpdateBatchDetails.Visible = false;

                        foreach (System.Web.UI.WebControls.ListItem li in BatchControlRblist.Items)
                        {
                            if (li.Text.ToLower() == "yes")
                            {
                                li.Selected = true;
                                PlaceHolderDivInAddItems.Visible = true;
                            }
                        }
                ...
                if (e.CommandName == "BatchOld")
                {
                    try
                    {
                        ...
                        //pre binding
                        LastbacthRecordToBatchFields(ID);
                        DivRbnBatchControlInNewInventoryInNewInventory.Visible = false;
                ...
                if (e.CommandName == "Update")
                {
                    CleargrdInventryFields();
                    int ID = Convert.ToInt32(e.CommandArgument.ToString());
                    int i = grdInventoryNew.EditIndex;
                    GridViewRow Row = grdInventoryNew.Rows[i];
                ...
                {
                    int ID = Convert.ToInt32(e.CommandArgument.ToString());
                    lblInventoryIdTodelete.Text = ID.ToString();
                    MdlPopUpForDeleteOption.Show();

                    //try
                    //{
                    //    IMCS = new InventoryManagerCs();
                    //    int retVal = -1;
                    //    DeleteBatchDataWithInventory(ID);
        ...
        protected void BtnYes_Click(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(lblInventoryIdTodelete.Text))
                {
                    int ID = 0;
                    bool check = int.TryParse(lblInventoryIdTodelete.Text, out ID);
                    if (check == true)
                    {
                        IMCS = new InventoryManagerCs();
                        int retVal = -1;
                        ...
                        InventoryBAL.DeleteInventoryJournal(ID);
                        InventoryGridBind();
                        lblInventoryIdTodeleteMsg.ForeColor = System.Drawing.Color.Red;
                        lblInventoryIdTodeleteMsg.Text = "Inventory deleted successfully";
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void BtnNo_Click(object sender, EventArgs e)
        {
            try
            {
                MdlPopUpForDeleteOption.Hide();
                lblInventoryIdTodelete.Text = string.Empty;
            }
            catch (Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
        ...
        public void DeleteBatchDataWithInventory(int InventoryId)
        {
            try
            {
                using (InventoryDataContext Idc = new InventoryDataContext())
                {
                    var Blist = Idc.Inventory_Batches.Where(a => a.InventoryID == InventoryId).ToList();
                    foreach (var x in Blist)
                    {
                        var BDatalist = Idc.Inventory_BatchCustomDatas.Where(a => a.BatchID == x.ID).ToList();
                        Idc.Inventory_BatchCustomDatas.DeleteAllOnSubmit(BDatalist);
                        Idc.SubmitChanges();
                    }
                    Idc.Inventory_Batches.DeleteAllOnSubmit(Blist);
                    Idc.SubmitChanges();
                }
            }
            catch (Exception ex)
        ...
        private void BindInventoryDetails()
        {
            using (InventoryDataContext db = new InventoryDataContext())
            {
                ...
                {
                    //GetQtyAvailable.....
                    lblCurrentQty.Text = getActualStackLaevelInMainGrid(inventory.Id.ToString()).ToString();
        ...
        public bool ChkMaterialDesc(int PortfolioID, string Material, int Category, int SubCategory, int ProductID, int SiteId)
        {
            object retObj = 0;
            bool re = false;
            try
            {
                ...
                IinventoryRepository<InventoryMgt.Entity.InventoryManager> irepository = new InventoryRepository<InventoryMgt.Entity.InventoryManager>();
                if (ProductID > 0)
                {
                    var iItem = irepository.GetAll().Where(o => o.Id != ProductID &&
                        o.ItemDescription.ToLower().Trim() == Material.ToLower().Trim()
                        && o.Categoery == Category && o.SubCategory == SubCategory && o.SiteId == SiteId).FirstOrDefault();
                    if (iItem != null)
                    {
                        re = true;
                    }
                ...
                {
                    var iItem = irepository.GetAll().Where(o => o.ItemDescription.ToLower().Trim() == Material.ToLower().Trim()
                        && o.Categoery == Category && o.SubCategory == SubCategory && o.SiteId == SiteId).FirstOrDefault();
                    if (iItem != null)
                    {
                        re = true;
        ...
        private void UpdateInventorySiteStorageDetails(int inventoryID, int categoryID, int subCategoryID, int siteID, int qty, string Type, int statusType)
        {
            try
            {
                if (Type == statusType.ToString() || statusType == 0)
                {
                    if (Type != "3")
                    {
                        using (InventoryDataContext db = new InventoryDataContext())
                        {
                            if (qty > 0)
                            {
                                var currentStorageList = db.Inventory_SiteStorageDetails.Where(i => i.InventoryId == inventoryID).OrderBy(i => i.UseStatus).Select(i => i).Take(qty).ToList();
                                int count = currentStorageList.Count();
                                foreach (var x in currentStorageList)
                                {
                                    x.UseStatus = true;
                                    db.SubmitChanges();
                                }
                            ...
                            {
                                qty = -qty;
                                var currentStorageList = db.Inventory_SiteStorageDetails.Where(i => i.InventoryId == inventoryID).Where(a => a.UseStatus == true).Select(i => i).Take(qty).ToList();
                                int count = currentStorageList.Count();
                                foreach (var x in currentStorageList)
                                {
                                    x.UseStatus = false;
                    ...
                    else
                    {
                        using (InventoryDataContext db = new InventoryDataContext())
                        {
                            List<Inventory_SiteStorageDetail> iList = new List<Inventory_SiteStorageDetail>();
                            if (qty > 0)
                            {
                                for (int i = 0; i < qty; i++)
                                {
                            ...
                            {
                                int deleteCount = -qty;
                                var currentStorageList = db.Inventory_SiteStorageDetails.Where(i => i.InventoryId == inventoryID).ToList();
                                var list = currentStorageList.Where(a => a.UseStatus == false).Take(deleteCount).ToList();
                                if (list != null)
                                {
                ...
                else
                {
                    if (qty > 0)
                    {
                        ...
                        else if (Type == "1" || Type == "2")
                        {
                            using (InventoryDataContext db = new InventoryDataContext())
                            {
                                var currentStorageList = db.Inventory_SiteStorageDetails.Where(i => i.InventoryId == inventoryID).Where(a => a.UseStatus == false).Select(i => i).Take(qty).ToList();
                                int count = currentStorageList.Count();
                                foreach (var x in currentStorageList)
                                {
                                    x.UseStatus = true;
            ...
            try
            {
                if (qty >= 0)
                {
                    using (InventoryDataContext db = new InventoryDataContext())
                    {
                        ...
                        var currentStorageList = db.Inventory_SiteStorageDetails.Where(i => i.InventoryId == inventoryID).Where(a => a.UseStatus == false).OrderBy(i => i.UseStatus).Select(i => i).ToList();
                        int count = currentStorageList.Count();
                        if (count == 0)
                        {
                            for (int i = 0; i < qty; i++)
            ...
            if (Convert.ToInt32(ddlCategory.SelectedValue) > 0)
            {
                if (Convert.ToInt32(ddlSubCategory.SelectedValue) > 0)
                {
                    if (Convert.ToInt32(ddlSite.SelectedValue) > 0)
                    {
                        ...
                        //While inserting data product id should be zero
                        if (ChkMaterialDesc(PortfolioID, txtItemDesc.Text,
                            Convert.ToInt32(ddlCategory.SelectedValue),
                            Convert.ToInt32(ddlSubCategory.SelectedValue), 0, Convert.ToInt32(ddlSite.SelectedValue)) == true)
                        {
                            try
                            {
                                int i = 0;
                        ...
                        {
                            lblError.Visible = true;
                            lblError.Text = "Product description already exists";
                        }
                    }
                    else
                    {
        ...
        protected void imgbtnUpdateMaterial_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_dt.Rows.Count > 0)
                {
                    ...
                    int SubCategory = int.Parse(_dr["SubCategory"].ToString());
                    int SiteId1 = int.Parse(_dr["SiteId"].ToString());
                    PortfolioID = int.Parse(ddlCustomer.SelectedValue);
                    bool saved = false;
                    if (ChkMaterialDesc(PortfolioID, txtItemDesc.Text, Category, SubCategory, ID, SiteId1) == false)
                    {
                        if ((pnlAddProduct.Visible == true) && (!saved))
                        {
                            string supplier = "";
                            ...
                            if (!string.IsNullOrEmpty(supplier))
                            {
                                ...
                                Qty = Convert.ToInt32((string.Empty == txtStock.Text.Trim() ? "0" : txtStock.Text.Trim()));
                                ...
                                using (InventoryDataContext db = new InventoryDataContext())
                                {
                                    ...
                                    Supplier, Manufacturer, DateOrdered, barcode, notes, int.Parse(ddlarea.SelectedValue), int.Parse(ddllocation.SelectedValue),
                                    int.Parse(ddlshelf.SelectedValue), int.Parse(ddlbin.SelectedValue), PartNumber);
                                    InventoryMgt.Entity.InventoryManager inventoryManager = db.InventoryManagers.Where(m => m.Id == ID).Select(m => m).FirstOrDefault();

                                    //reorder level cheking
                                    ReorderStockItemMail(ID);
                    ...
                    else
                    {
                        ...
                        lblMessage.Text = "Product description already exists.";
                    }
                }
            }
            catch (Exception ex)
        ...
        private bool InventoryDetailsChange(int id, int qty)
        {
            ...
            using (InventoryDataContext db = new InventoryDataContext())
            {
                InventoryManager inventoryManager = db.InventoryManagers.Where(i => i.Id == id).Select(i => i).FirstOrDefault();
                if (inventoryManager != null)
                {
        ...
        protected void imgbtnViewInven_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        protected void btnAddNewItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                txtProductEdit.Text = string.Empty;
                txtQtyEdit.Text = string.Empty;
                TxtproductInNewItem.Text = string.Empty;
                txtQtyNewItem.Text = string.Empty;
                if (sessionKeys.PortfolioID > 0)
                {
                    ddlcustomerInNewItem.SelectedValue = sessionKeys.PortfolioID.ToString();
                    BindSiteInNewItem();
                    //setting value from search
                    if (int.Parse(ddlsiteInSearch.SelectedValue) > 0)
                    {
                        ddlsiteInNewItem.SelectedValue = ddlsiteInSearch.SelectedValue;
                    }
                    BindCategoryInNewItem(sessionKeys.PortfolioID);
                    //setting value from search
                    if (int.Parse(ddlcategoryInSearch.SelectedValue) > 0)
                    {
                        ddlcategoryInNewItem.SelectedValue = ddlcategoryInSearch.SelectedValue;
                    }
                    BindSubCategoryInNewItem(int.Parse(ddlcategoryInNewItem.SelectedValue));
                    //setting value from search
                    if (int.Parse(ddlsubcategoryInSearch.SelectedValue) > 0)
                    {
                        ddlSubcategoryInNewItem.SelectedValue = ddlsubcategoryInSearch.SelectedValue;
                    }
                ...
                DivRbnBatchControlInNewInventoryInNewInventory.Visible = true;
            }
            catch (Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
        ...
        private void InventoryUpload(string type, string path, string fileName, string conStr)
        {
            try
            {
                ...
                //int siteId = int.Parse(ddlSite.SelectedValue);
                int customer = sessionKeys.PortfolioID;
                int siteId = int.Parse(ddlsiteInSearch.SelectedValue);
                ...
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    ...
                    if (inventoryList.Count() > 0)
                    {
                        ...
                        //finally dt datatable is barcodes
                        var result = from c in dt.AsEnumerable().ToList()
                                     group c by new
                                     {
                                         BarCode = c.Field<string>("BarCode"),
                                         UseCode = c.Field<string>("UseCode"),
                                         UseID = c.Field<int>("UseID")
                                     } into g
                                     select new
                                     {
                        ...
                        foreach (var item in result)
                        {
                            ...
                            var barcodeExists = (from b in iList
                                                 ...
                                                 group b by new
                                                 {
                                                     Product = b.i.ItemDescription,
                                                     ID = b.s.InventoryId,
                                                     Category = b.i.CategoryName,
                                                     SubCategory = b.i.SubCategoryName,
                                                     Site = b.i.SiteName
                                                 } into grouping
                                                 select new
                                                 {
                                                     Product = grouping.Key.Product,
                                                     QtyOpen = grouping.Count(),
                                                     QtyDeployed = item.ItemCount,
                                                     ID = grouping.Key.ID,
                                                     Use = item.Use,
                                                     UseID = item.UseID,
                                                     Category = grouping.Key.Category,
                                                     SubCategory = grouping.Key.SubCategory,
                        ...
                        foreach (var item in upl)
                        {
                            ...
                            if (sList.Count() > 0)
                            {
                                ...
                                //Updating usage and batch tables
                                var u1 = useList.Where(a => a.Code == item.Use).FirstOrDefault();
                                string UseName = u1.Name + "(" + u1.Code + ")";
                                UpdateUsageBatchTables(item.InventoryID, sList.Count.ToString(), UseName);

                                InventoryManager inventoryManager = InventoryBAL.GetInventoryByID(item.InventoryID);
                                if (inventoryManager != null)
                                {
        ...
        public void UpdateUsageBatchTables(int Inventory_Id, string qty, string UseName)
        {
            try
            {
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    var AllBatchs = db.Inventory_Batches.Where(a => a.InventoryID == Inventory_Id).OrderBy(a => a.ID).ToList();
                    int NewQty = 0;
                    bool IsFirstTime = true;
                    int FirstBatchId = AllBatchs.FirstOrDefault().ID;
                    List<InventoryManager_PSDProduct> PSDList1 = new List<InventoryManager_PSDProduct>();
                    PSDList1 = db.InventoryManager_PSDProducts.Where(p => p.ProductId == Inventory_Id).ToList();
                    int BAQty1 = AllBatchs.FirstOrDefault().QTY.Value - PSDList1.Where(o => o.BatchID == FirstBatchId).Select(o => o.QtyUsed.Value).Sum();
                    if (BAQty1 >= int.Parse(qty))
                    {
                        UpdatingUsageTables(Inventory_Id, int.Parse(qty), AllBatchs.FirstOrDefault().ID, UseName);
                    }
                    else
                    {
                        foreach (var x in AllBatchs)
                        {
                            int BAQty = x.QTY.Value - PSDList1.Where(o => o.BatchID == x.ID).Select(o => o.QtyUsed.Value).Sum();
                            if (IsFirstTime == true)
                            {
                                if (BAQty > 0)
                                {
                                    UpdatingUsageTables(Inventory_Id, BAQty, x.ID, UseName);
                            ...
                            else
                            {
                                if (NewQty > 0)
                                {
                                    if (BAQty > 0)
                                    {
                                        UpdatingUsageTables(Inventory_Id, int.Parse(NewQty.ToString()), x.ID, UseName);
        ...
        public void UpdatingUsageTables(int InventoryId, int Qty, int BatchId, string UseName)
        {
            try
            {
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    ...
                    Im_Usage.projectid = 0;
                    Im_Usage.BatchID = BatchId;
                    Im_Usage.notes = UseName;// "Deployed By " + sessionKeys.UName;
                    Im_Usage.SectionType = "PROJECT";
                    //  Im_Usage.Requester = UseName;
        ...
        private void WriteUploadExeceptionReport(XLWorkbook wb)
        {
            try
            {
                string pathLocation = HttpContext.Current.Server.MapPath("~/WF/UploadData/InventoryExeceptionReport");

                if (Directory.Exists(pathLocation) == false)
                {
                    Directory.CreateDirectory(pathLocation);
                }
                string file = ddlcustomerInSearch.SelectedItem.Text + "_Inventory_Upload_" + string.Format("{0:ddMMyyy HHmmss}", DateTime.Now) + ".xlsx";
                ...
                if (fileInfo.Exists)
                {
                    ...
                    AC2PDocumentsController.DN_ProjectUploadInsertNew(QueryStringValues.Project.ToString(), file, document, file, "application/ms-excel", "P", Convert.ToInt32(fileInfo.Length), folderId, sessionKeys.UID, sessionKeys.IncidentID);
        ...
        protected void ImageButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (IsValid(fileUpload1.PostedFile.FileName))
                {
                    ...
                    string path = Server.MapPath("~/WF/UploadData/Inventory");
        ...
        public void UpdateBatchInTransForQty(int InventoryId, int Qty)
        {
            try
            {
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    Inventory_Batch In_B = db.Inventory_Batches.Where(a => a.InventoryID == InventoryId).FirstOrDefault();
                    if (In_B != null)
                    {
                        In_B.QTY = In_B.QTY.Value + Qty;
                        In_B.DateReceived = DateTime.Now;
                        db.SubmitChanges();
                    }
                }
            }
        ...
        protected void imgTransferApply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    ...
                    if (siteId == 0)
                    {
                        if (imgr != null)
                        {
                            ...
                            //Update batch 
                            UpdateBatchInTransForQty(imgr.Id, transferQty);

                            //int openingStock=GetQtyAvailable(imgr.Id.ToString());
                            //April 22nd modified
                            int openingStock = getActualStackLaevelInMainGrid(imgr.Id.ToString());
                    ...
                    else
                    {
                        ...
                        if (int.Parse(hfSiteID.Value) != siteId)
                        {
                            ...
                            if ((currentQty >= int.Parse(amendment)))
                            {
                                ...
                                var sList = db.Inventory_SiteStorageDetails.Where(s => s.InventoryId == int.Parse(hfInventoryId.Value)
                                    && s.UseStatus == false).Select(s => s).Take(int.Parse(amendment)).ToList();
                                InventoryMgt.Entity.InventoryManager im = inventoryList.Where(m => m.ItemDescription == hfDescription.Value
                                    && m.PortfolioID == sessionKeys.PortfolioID
                                    && m.Categoery == int.Parse(hfCategoryId.Value) && m.SubCategory == int.Parse(hfSubcategoryId.Value)
                                    && m.SiteId == siteId).FirstOrDefault();
                                //reduce amendment from previoes inventory
                                UpdateUsageBatchTables(int.Parse(hfInventoryId.Value), amendment.ToString(), sessionKeys.UName);
                                if (im != null)
                                {
                                    ...
                                    //Update batch 
                                    UpdateBatchInTransForQty(im.Id, int.Parse(amendment));


                                    //Journal Inventory Manager insert
                                    InventoryManager getCurrentInventory = InventoryBAL.GetInventoryByID(im.Id);
                                ...
                                else
                                {
                                    ...
                                    db.SubmitChanges();
                                    Inventory_Batch_Add(inventoryManager.Id, int.Parse(amendment), true);
                                    ...
                                    //Journal Inventory Manager insert
                                    InventoryManager getCurrent = InventoryBAL.GetInventoryByID(inventoryManager.Id);
                                    if (getCurrent != null)
                                    {
                                        getCurrent.Notes = amendment + " Items transferred from " + InventoryBAL.GetSiteNameByID(Convert.ToInt32(imgr.SiteId));
                                        // getCurrent.Qty = GetQtyAvailable(getCurrent.Id.ToString());
                ...
                using (InventoryDataContext db = new InventoryDataContext())
                {
                    using (UserDataContext ud = new UserDataContext())
                    {
                        ...
                        var usageReport = (from i in inventoryJournalList
                                           ...
                                           join c in categoryList on p.Categoery equals c.ID
                                           join sc in categoryList on p.SubCategory equals sc.ID
                                           join s in siteList on p.SiteId equals s.ID
                        ...
                        if (typeOfExport == "Excel")
                        {
                            ...
                            // Title
                            ws.Cell("A1").Value = "Customer:";
                            ws.Cell("B1").Value = ddlcustomerInSearch.SelectedItem.Text;
                            ws.Cell("C1").Value = "Site:";
                            ws.Cell("D1").Value = ddlsiteInSearch.SelectedItem.Text;
                            ...
                            string path = HttpContext.Current.Server.MapPath("~/WF/UploadData/Inventory");
        ...
        protected void grdInventory_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    LinkButton imgDelete = ((LinkButton)e.Row.FindControl("LinkButtonDelete1"));
                    ...
                    //LinkButton lnkBatch = ((LinkButton)e.Row.FindControl("lnkBatch"));
                    //if(hBatchoption.Value != "")
                    //{
                    //    lnkBatch.Visible = Convert.ToBoolean(hBatchoption.Value == string.Empty ? "True" : hBatchoption.Value);
                    //}
                }
                ...
                // gv = (GridView)row.FindControl("GridBatchInMainGrid");
                ...
                // var rowView = (InventorySearch)e.Row.DataItem;
                // int ProductId = rowView.Id;
                //// int ProductId = Convert.ToInt32(rowView["id"].ToString());

                // // gv.DataSource = BindBatchsInmainGrid(ProductId);
                // // gv.DataBind();


                //  GridView gvUsage = new GridView();
                //  gvUsage = (GridView)row.FindControl("GridusageInMainGrid");
                //  if (gvUsage.UniqueID == gvUniqueID)
                //  {
                //      gvUsage.PageIndex = gvNewPageIndex;
                //      gvUsage.EditIndex = gvEditIndex;
                //      //Check if Sorting used
                //  }
                //  gvUsage.DataSource = Bind_UsagesInMainGrid(ProductId);
                //  gvUsage.DataBind();
        ...
        public void BindRelatedSitesGrid(GridView gv, int InventoryId)
        {
            try
            {
        ...
        public bool ContainsOfIn_IdInMainGrid(List<int> IntCollection, int In_Id)
        {
            bool IsExist = false;
            try
            {
        ...
        public void InventoryInnerGridBind(string In_Description,GridView Gd)
        {
            try
            {
                ...
                imlist = imlist.Where(Im => Im.ItemDescription.ToLower() == In_Description.ToLower()).ToList();
                ...
                var r1 = (from x in imlist
                          select new InventorySearch
                          {
                              ...
                              AreaName = x.Area.HasValue ? arealist.Where(o => o.id == x.Area).Select(o => o.Name).FirstOrDefault() : string.Empty,
                              LocationName = x.Location.HasValue ? llist.Where(o => o.id == x.Location).Select(o => o.Name).FirstOrDefault() : string.Empty,
                              ShelfName = x.Shelf.HasValue ? slist.Where(o => o.id == x.Shelf).Select(o => o.Name).FirstOrDefault() : string.Empty,
                              BinName = x.Bin.HasValue ? blist.Where(o => o.id == x.Bin).Select(o => o.Name).FirstOrDefault() : string.Empty,
                ...
                {
                    Gd.Columns[Gd.Columns.Count - 1].Visible = true;
                    Gd.Columns[Gd.Columns.Count - 2].HeaderStyle.CssClass = "";
                }

                if (sessionKeys.SID == 4)
                {
                    Gd.Columns[0].Visible = false;
                    Gd.Columns[1].HeaderStyle.CssClass = "header_bg_l";
                    Gd.Columns[Gd.Columns.Count - 2].Visible = false;
                    Gd.Columns[Gd.Columns.Count - 2].HeaderStyle.CssClass = "";
                }
                else
                {
                    using (UserDataContext um = new UserDataContext())
                    {
                        UserDetail ud = (from p in um.UserDetails
                                         where p.UserId == sessionKeys.UID
                                         select p).FirstOrDefault();
                        if (ud.UsercandeleteitemsfromtheInventory == true)
                        {
                            Gd.Columns[Gd.Columns.Count - 2].Visible = true;
                            Gd.Columns[Gd.Columns.Count - 2].HeaderStyle.CssClass = "";
                        }
                    }
                }
        ...
        private void InventoryGridBind(bool isInserted = false)
        {
            try
            {
                ...
                if (sessionKeys.PortfolioID > 0)
                {
                    imlist = imlist.Where(o => o.PortfolioID == sessionKeys.PortfolioID).ToList();
                    //filter by site
                    if (!string.IsNullOrEmpty(ddlsiteInSearch.SelectedValue))
                    {
                        if (Convert.ToInt32(ddlsiteInSearch.SelectedValue) > 0)
                        {
                            imlist = imlist.Where(o => o.SiteId == Convert.ToInt32(ddlsiteInSearch.SelectedValue)).ToList();
                        }
                    }
                    //filter by category
                    if (!string.IsNullOrEmpty(ddlcategoryInSearch.SelectedValue))
                    {
                        if (Convert.ToInt32(ddlcategoryInSearch.SelectedValue) > 0)
                        {
                            imlist = imlist.Where(o => o.Categoery == Convert.ToInt32(ddlcategoryInSearch.SelectedValue)).ToList();

                            //filter by sub category
                            if (!string.IsNullOrEmpty(ddlsubcategoryInSearch.SelectedValue))
                            {
                                if (Convert.ToInt32(ddlsubcategoryInSearch.SelectedValue) > 0)
                                {
                                    imlist = imlist.Where(o => o.SubCategory == Convert.ToInt32(ddlsubcategoryInSearch.SelectedValue)).ToList();
                                }
                            }
                        }
                    }
                ...
                List<int> InventoryIdCollection = new List<int>();
                var ItemsDesGruoplist = (from sr in imlist
                                         group sr by sr.ItemDescription into ItemDgroup
                                         orderby ItemDgroup.Key
                                         select new
                                         {
                                             key = ItemDgroup.Key,
                                             InColl = ItemDgroup
                                         }).ToList();
                foreach (var g in ItemsDesGruoplist)
                {
                    foreach (var g1 in g.InColl)
                    {
                        InventoryIdCollection.Add(g1.Id);
                        break;
                    }
                }
                imlist = imlist.Where(Im => ContainsOfIn_IdInMainGrid(InventoryIdCollection, Im.Id)).ToList();
                ...
                var r1 = (from x in imlist
                          select new InventorySearch
                          {
                              ...
                              ManufacturerName = mlist.Where(o => o.Id == x.Manufacturer).Select(o => o.Name).FirstOrDefault(),
                              Sitename = sitelist.Where(o => o.id == x.SiteId).Select(o => o.Site).FirstOrDefault(),
                              AreaName = string.Empty,// arealist.Where(o => o.id == x.Area).Select(o => o.Name).FirstOrDefault(),
                              LocationName = string.Empty,//llist.Where(o => o.id == x.Location).Select(o => o.Name).FirstOrDefault(),
                              ShelfName = string.Empty,// slist.Where(o => o.id == x.Shelf).Select(o => o.Name).FirstOrDefault(),
                              BinName = string.Empty,// blist.Where(o => o.id == x.Bin).Select(o => o.Name).FirstOrDefault(),
                              Area = 0,// x.Area,
                ...
                if (!isInserted)
                {
                    if (stringvalue != string.Empty)
                    {
                        ...
                        if (Svalue.Length > 1)
                        {
                            ...
                            grdInventoryNew.DataSource = r1.Where(o => IntCollection.IndexOf(o.Id) >= 0).OrderBy(o => IntCollection.IndexOf(o.Id)).ToList();
                        ...
                        else
                        {
                            r1 = (from x in r1
                                  ...
                                  select x).ToList();
                            grdInventoryNew.DataSource = r1.OrderByDescending(o => o.Id).ToList();
                        }
                    }
                    else
                    {
                ...
                if (r1 != null)
                {
                    if (r1.Count > 0)
                    {
                        InventoryFirstId.Value = r1.OrderByDescending(o => o.Id).FirstOrDefault().Id.ToString();
                    }
                    else
                    {
                        InventoryFirstId.Value = "";
                    }
                }
                grdInventoryNew.DataBind();
                ...
                //if (sessionKeys.SID == 1)
                //{
                //    grdInventoryNew.Columns[grdInventoryNew.Columns.Count - 1].Visible = true;
                //    grdInventoryNew.Columns[grdInventoryNew.Columns.Count - 2].HeaderStyle.CssClass = "";
                //}
                //else
                //{
                //    using (UserDataContext um = new UserDataContext())
                ...
                //if (sessionKeys.SID == 4)
                //{
                //    grdInventoryNew.Columns[0].Visible = false;
                //    grdInventoryNew.Columns[1].HeaderStyle.CssClass = "header_bg_l";
                //}

                //batch option
               //  grdInventoryNew.Columns[13].Visible = Convert.ToBoolean(hBatchoption.Value == string.Empty ? "True" : hBatchoption.Value);

                //Hide cat,subCat,Manifacture
                //grdInventoryNew.Columns[2].Visible = false;
                //grdInventoryNew.Columns[3].Visible = false;
               // grdInventoryNew.Columns[5].Visible = false;
        ...
        protected void lnKBtnShowAllColumns_Click(object sender, EventArgs e)
        {
            try
            {
                if (lnKBtnShowAllColumns.Text == "Show all Columns")
                {
                    lnKBtnShowAllColumns.Text = "Show limited Columns";
                    grdInventoryNew.Columns[2].Visible = true;
                    grdInventoryNew.Columns[3].Visible = true;
                    grdInventoryNew.Columns[5].Visible = true;
                }
                else
                {
                    lnKBtnShowAllColumns.Text = "Show all Columns";
                    grdInventoryNew.Columns[2].Visible = false;
                    grdInventoryNew.Columns[3].Visible = false;
                    grdInventoryNew.Columns[5].Visible = false;
                }
        ...
        public void BindSiteInPopup()
        {
            try
            {
                SC_cntMaterialCS = new SC_ContractorMaterial();
                //ddlSite.DataSource =  SC_cntMaterialCS.GetSites();
                ddlsiteStockadd.DataSource = Deffinity.Bindings.DefaultDatabind.b_SiteSelect_Portfilio(sessionKeys.PortfolioID);
                ...
                if (SiteId > 0)
                {
                    ddlsiteStockadd.SelectedValue = SiteId.ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        protected void Imgbtnbinadd_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlshelfStockadd.SelectedValue != "0")
        ...
        protected void BtnUpdateBatchDetails_Click(object sender, EventArgs e)
        {
            UpdateBatchInAddStock();
        }
        ...
        protected void btnSubmitInPopUp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                In_batch = new InventoryRepository<Inventory_Batch>();
                int inventoryid = int.Parse(ddlProduct.SelectedValue);
                if (inventoryid == 0)
                {
                    inventoryid = int.Parse(LblInventoryId.Text);
                }

                int siteid = int.Parse(ddlsiteStockadd.SelectedValue);
                Inventory_New_Stock In_nsForCheck = new Inventory_New_Stock();
                In_nsForCheck = INS.GetAll().Where(a => a.ProductId == inventoryid && a.BatchControl == false).FirstOrDefault();
                Inventory_Batch In_BatchUpdate = new Inventory_Batch();
                In_BatchUpdate = In_batch.GetAll().Where(a => a.InventoryID == inventoryid && a.BatchControl == false).FirstOrDefault();

                if (In_nsForCheck != null && BatchControlRblist.SelectedItem.Text == "No")
                {
                    In_nsForCheck.Qty = In_nsForCheck.Qty + int.Parse(txtQuantity.Text);
                    if (!string.IsNullOrEmpty(txtnotesinpopup.Text))
                    {
                        In_nsForCheck.Notes = In_nsForCheck.Notes + "." + txtnotesinpopup.Text;
                    }
                    INS.Edit(In_nsForCheck);
                    In_BatchUpdate.QTY = In_BatchUpdate.QTY + int.Parse(txtQuantity.Text);
                    In_batch.Edit(In_BatchUpdate);
                }
                else
                {
                    In_ns.ProductId = inventoryid;
                    In_ns.SiteId = siteid;
                    ...
                    if (BatchControlRblist.SelectedItem.Text == "Yes")
                    {
                        In_ns.BatchControl = true;
                    }
                    else
                    {
                        In_ns.BatchControl = false;
                    }
                    INS.Add(In_ns);
                    //New code
                    int BatchId = 0;
                    if (BatchControlRblist.SelectedItem.Text == "Yes")
                    {
                        BatchId = Inventory_Batch_Add_InAddStock(inventoryid, int.Parse(txtQuantity.Text), true);
                    }
                    else
                    {
                        BatchId = Inventory_Batch_Add_InAddStock(inventoryid, int.Parse(txtQuantity.Text), false);
                    }
                    saveBatchCustomInAddStock(BatchId, inventoryid);
                }
               

                In_Ma = IM.GetAll().Where(o => o.Id == inventoryid).FirstOrDefault();
                int qty = int.Parse(In_Ma.Qty.ToString());
                In_Ma.Qty = In_Ma.Qty + int.Parse(txtQuantity.Text);
                IM.Edit(In_Ma);

                int OpeningStock = GetQtyAvailable(inventoryid.ToString());
                int Storage_Qty = GetQtyAvailable(inventoryid.ToString()) + int.Parse(txtQuantity.Text);

                AddInventorySiteStorageDetails(inventoryid, 0, 0, siteid, Storage_Qty);

                In_mj.InventoryId = inventoryid;
                In_mj.ProductId = inventoryid;
                In_mj.SiteId = siteid;
                ...
                In_mj.Qty = GetQtyAvailable(inventoryid.ToString());
                ...
                In_mj.UseID = 0;
                //if existing quntity is zero
                //it should set opening stock
                if (OpeningStock == 0)
                    In_mj.OpeningStock = int.Parse(txtQuantity.Text);
                else
                    In_mj.TransferQty = int.Parse(txtQuantity.Text);

                In_mj.ModifiedBy = sessionKeys.UID;
                In_mj.ReasonCode = "New Stock";
                In_mj.MdofiedDate = DateTime.Now;
                In_mj.QtyChangedOrNot = true;
                IMJ.Add(In_mj);
                ...
                //Grid bind after new stock added
                BindBatchGrid();
                ...
                BindSiteInPopup();
                BindAreaInPopup(int.Parse(ddlCustomer.SelectedValue));
                BindPopUpGridItems(inventoryid);
                BindProductsSection(inventoryid.ToString());
                // UpdatePanel2.DataBind();
                mdlpopupaddnewstock.Show();
                InventoryGridBind();
                pnlUseShow.Visible = false;
            }
            catch (Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
        ...
        public int Inventory_Batch_Add_InAddStock(int inventoryID, int Qty, bool BatchControl)
        {
            ...
            ib.SiteId = Convert.ToInt32(!string.IsNullOrEmpty(ddlsiteStockadd.SelectedValue) ? ddlsiteStockadd.SelectedValue : "0");
            ib.Area = Convert.ToInt32(!string.IsNullOrEmpty(ddlareaStockadd.SelectedValue) ? ddlareaStockadd.SelectedValue : "0");
            ib.Location = Convert.ToInt32(!string.IsNullOrEmpty(ddllocationStockadd.SelectedValue) ? ddllocationStockadd.SelectedValue : "0");
            ib.Shelf = Convert.ToInt32(!string.IsNullOrEmpty(ddlshelfStockadd.SelectedValue) ? ddlshelfStockadd.SelectedValue : "0");
            ib.Bin = Convert.ToInt32(!string.IsNullOrEmpty(ddlbinStockadd.SelectedValue) ? ddlbinStockadd.SelectedValue : "0");
            ib.BatchControl = BatchControl;
            batchRepository.Add(ib);
            batchRepository.Save();
            return ib.BatchRef.Value;
        }
            ...
            try
            {
                ...
                var result = (from a in NewStocklist
                              select new
                              {
                                  ...
                                  BatchControlYesOrNo = a.BatchControl.HasValue ? a.BatchControl == true ? "Yes" : "No" : "Yes"
                              }).ToList();
        ...
        protected void gridNewStockItems_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "Delete")
                {
                    ...
                    int ActualStock = getActualStackLaevelInMainGrid(x.ProductId.ToString());
        ...
        protected void hlinkExecption_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/WF/Projects/ProjectDocuments.aspx?mode=central&folderID=" + folderId + "&type=in");
            //  Session["behcode"] = 111;
        ...
        protected void ImageButtonBinEdit_Click(object sender, EventArgs e)
        {
        ...
        protected void ImageButtonbinAdd_Click(object sender, EventArgs e)
        {
            if (ddlshelf.SelectedValue != "0")
            {
                txtBinName.Text = string.Empty;
        ...
        {
            Response.Redirect("~/WF/Admin/AdminDropdown.aspx?Panel=9");
        }
            ...
            try
            {
                if (FilterdBatchGrid.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (Idc.InventoryManager_PSDProducts.Where(a => a.BatchID == lblBId && a.Id != int.Parse(pid)).Count() != 0)
                            {
                        ...
                        if (BatchAvaQty >= int.Parse(DeployQty))
                        {
                            ...
                            IM_Products.projectid = 0;// int.Parse(ProjectId);
                            IM_Products.SectionType = "PROJECT";
                            int QtyUsed = IM_Products.QtyUsed.Value;
                            int StatusType = IM_Products.Status.Value;
                            ...
                            IM_Products.RecordNumber = txtRecordNumber.Text.Trim();

                            int MAxQtyValue = getActualStackLaevelInMainGrid(ddlProduct.SelectedValue);
                            ...
                            if (status != "3")
                            {
                                ...
                                {
                                    Idc.SubmitChanges();
                                    LblListviewMsg.ForeColor = System.Drawing.Color.Green;
                                    LblListviewMsg.Text = "Updated sucessfully";
                                    //update storage details
                                    UpdateInventorySiteStorageDetails(IM_Products.ProductId.Value, 0, 0, 0, int.Parse(DeployQty) - QtyUsed, ddlStatus.SelectedValue, StatusType);

                                    //Journal Entry
                                    AddInventoryJournalInUpdateUsage(IM_Products.ProductId.Value, int.Parse(DeployQty) - QtyUsed, IM_Products.QtyUsed.Value);

                                    CopyBatchCustomFields(IM_Products.Id, lblBId);
                                    BindProductsSection(ddlProduct.SelectedValue);
                                    mdlpopupinGrid.Hide();
                                    FieldsClear();
                                    BindData_UsageGrid();
                                ...
                                {
                                    LblListviewMsg.ForeColor = System.Drawing.Color.Red;
                                    LblListviewMsg.Text = "Quantity exceeds, Please check available quantity";
                                    mdlpopupinGrid.Show();
                                }
                            }
                            else
                            {
                                ...
                                {
                                    InventoryManager In_Manager = Idc1.InventoryManagers.Where(a => a.Id == IM_Products.ProductId.Value).FirstOrDefault();
                                    In_Manager.Qty = In_Manager.Qty + int.Parse(DeployQty) - QtyUsed;
                                    Idc1.SubmitChanges();
                                }
                                //update storage details
                                UpdateInventorySiteStorageDetails(IM_Products.ProductId.Value, 0, 0, 0, int.Parse(DeployQty) - QtyUsed, ddlStatus.SelectedValue, StatusType);
                                ...
                                //Journal Entry
                                AddInventoryJournalInUpdateUsage(IM_Products.ProductId.Value, int.Parse(DeployQty), IM_Products.QtyUsed.Value);
                            ...
                            if (IM_Products.ProductId > 0)
                            {
                                var inventoryItem = Idc.InventoryManagers.Where(o => o.Id == IM_Products.ProductId.Value).FirstOrDefault();
                                ReorderStockItemMail(inventoryItem.Id);
                            }
                        }
                        else
                        {
                            LblListviewMsg.ForeColor = System.Drawing.Color.Red;
                            LblListviewMsg.Text = "Quantity exceeds, please select other batch";
                            mdlpopupinGrid.Show();
                        }
                    }
                }
        ...
        protected void Btninsert_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DivNonBatchControlValues.Visible == true)
                {
                    lblBId = int.Parse(lblDefaultBatchId.Text);
                }
                ...
                else
                {
                    ...
                    if (BatchAvaQty >= int.Parse(DeployQty))
                    {
                        ...
                        MAxQtyValue = getActualStackLaevelInMainGrid(Proid);
                        InventoryManager_PSDProduct IM_Products = new InventoryManager_PSDProduct();
                        ...
                        if (status != "3")
                        {
                            if (MAxQtyValue >= int.Parse(DeployQty))
                            {
                                using (InventoryDataContext Idc = new InventoryDataContext())
                                {
                                    ...
                                    //update storage details
                                    UpdateInventorySiteStorageDetails(IM_Products.ProductId.Value, 0, 0, 0, int.Parse(DeployQty), ddlStatus.SelectedValue, 0);

                                    AddInventoryJournalInUsage(IM_Products.ProductId.Value, int.Parse(DeployQty));
                                    int pid = IM_Products.Id;
                                    CopyBatchCustomFields(pid, lblBId);
                        ...
                        else
                        {
                            using (InventoryDataContext Idc1 = new InventoryDataContext())
                            {
                                ...
                                //Inventory manager
                                InventoryManager In_Manager = Idc1.InventoryManagers.Where(a => a.Id == IM_Products.ProductId.Value).FirstOrDefault();
                                In_Manager.Qty = In_Manager.Qty + int.Parse(DeployQty);
                                Idc1.SubmitChanges();
                                ...
                                if (IM_Products.ProductId > 0)
                                {
                                    var inventoryItem = Idc1.InventoryManagers.Where(o => o.Id == IM_Products.ProductId.Value).FirstOrDefault();
                                    ReorderStockItemMail(inventoryItem.Id);
                                }
                            }
                            //Listview.EditIndex = -1;
        ...
        public void AddInventoryJournalInUsage(int ProductID, int Qty)
        {
            try
            {
                IMJ = new InventoryRepository<InventoryManagerJournal>();
                ...
                In_mj.Qty = GetQtyAvailable(ProductID.ToString());
                In_mj.SectionType = "Global";
                In_mj.PortfolioID = sessionKeys.PortfolioID;
                In_mj.DateOrdered = DateTime.Now.Date;
                //
                In_mj.SiteId = IM.GetAll().Where(a => a.Id == ProductID).Select(a => a.SiteId).FirstOrDefault();
                In_mj.UseID = 0;
                In_mj.ModifiedBy = sessionKeys.UID;
                In_mj.ReasonCode = "Usage(" + ddlStatus.SelectedItem.Text + ")";
                In_mj.MdofiedDate = DateTime.Now;
                In_mj.OpeningStock = IMJ.GetAll().Where(a => a.InventoryId == ProductID).OrderByDescending(a => a.Id).Select(a => a.Qty.HasValue ? a.Qty.Value : 0).FirstOrDefault();
                In_mj.TransferQty = Qty;
                In_mj.QtyChangedOrNot = true;
                IMJ.Add(In_mj);
            }
            catch (Exception ex)
        ...
        public void AddInventoryJournalInUpdateUsage(int ProductID, int Qty, int PreviousQty)
        {
            try
            {
                IMJ = new InventoryRepository<InventoryManagerJournal>();
                ...
                In_mj.UseID = 0;
                In_mj.ModifiedBy = sessionKeys.UID;
                In_mj.ReasonCode = "Usage(" + ddlStatus.SelectedItem.Text + ") updated";
                In_mj.MdofiedDate = DateTime.Now;
                In_mj.OpeningStock = IMJ.GetAll().Where(a => a.InventoryId == ProductID).OrderByDescending(a => a.Id).Select(a => a.Qty.HasValue ? a.Qty.Value : 0).FirstOrDefault();
                In_mj.TransferQty = Qty;
                In_mj.QtyChangedOrNot = true;
        ...
        public void CopyBatchCustomFields(int pid, int lblBId)
        {
            try
            {
                IUCD = new InventoryRepository<InventoryManager_UsageCustomData>();
                ...
                In_IBCD = new Inventory_BatchCustomData();

                var UsageFiledsList = IUCD.GetAll().Where(a => a.PSDID == pid).ToList();
                var batchFieldsList = IBCD.GetAll().Where(a => a.BatchID == lblBId).ToList();
                foreach (var x in batchFieldsList)
                {
                    if (UsageFiledsList.Where(a => a.CustomFieldID == x.CustomFieldID).FirstOrDefault() == null)
                    {
                        iuc = new InventoryManager_UsageCustomData();
                        iuc.CustomFieldID = x.CustomFieldID;
                        iuc.CustomFieldValue = x.CustomFieldValue;
                        iuc.PSDID = pid;
                        IUCD.Add(iuc);
                    }
                    else
                    {
                        iuc = new InventoryManager_UsageCustomData();
                        iuc = UsageFiledsList.Where(a => a.CustomFieldID == x.CustomFieldID).FirstOrDefault();
                        iuc.CustomFieldID = x.CustomFieldID;
                        iuc.CustomFieldValue = x.CustomFieldValue;
                        iuc.PSDID = pid;
                        IUCD.Edit(iuc);
        ...
        public void FieldsClear()
        {
            ...
            txtNote.Text = string.Empty;
            txtRecordNumber.Text = string.Empty;
            ChcekFromBacthControl.Checked = false;
        }
            ...
            try
            {
                ...
                foreach (var d in InList)
                {
                    ...
                    foreach (var p in plist)
                    {
                        ...
                        {
                            var strs = new string[] { "Qty Used", "Requester", "Project", "Status", "Notes", "Record Number" };
            ...
            try
            {
                if (e.CommandName == "Edit1")
                {
                    ...
                    using (InventoryDataContext Idc = new InventoryDataContext())
                    {
                        ...
                        var VList = (from a in Idc.InventoryManager_PSDProducts
                                     //join b in plist on a.projectid equals b.ProjectReference
                                     where a.ProductId == int.Parse(ddlProduct.SelectedValue) && a.Id == id
                                     select new
                                     {
                                         productId = a.ProductId,
                                         pid = a.projectid,
                                         projectid = string.Empty, //b.ProjectReferenceWithPrefix + " - " + b.ProjectTitle,
                                         Id = a.Id,
                                         QtyUsed = a.QtyUsed,
                        ...
                        {
                            int Testvalue = 0;
                            if (VList.FirstOrDefault().batchid.HasValue)
                            {
                                Testvalue = VList.Select(o => o.batchid.Value).FirstOrDefault();
                            }
                            // ddlBatch.SelectedValue = Testvalue.ToString();
                            ...
                            txtRecordNumber.Text = VList.Select(a => a.RecordNumber).FirstOrDefault();
                            ...
                            string NonBatchControl = string.Empty;
                            ...
                            if (FilterdBatchGrid.Rows.Count > 0)
                            {
                                for (int i = 0; i < FilterdBatchGrid.Rows.Count; i++)
                                {
                                    ...
                                    if (lblBId == VList.Select(o => o.batchid).FirstOrDefault())
                                    {
                                        ...
                                        NonBatchControl = "Checked";
                                        break;
                                    }
                                }
                            }
                            if (NonBatchControl == string.Empty)
                            {
                                ChcekFromBacthControl.Checked = true;
                            }
                            else
                            {
                                ChcekFromBacthControl.Checked = false;
        ...
        public void CheckingDefaultBatch()
        {
            ...
            try
            {
                using (InventoryDataContext Idc = new InventoryDataContext())
                {
                    ...
                    {
                        hBatchoption.Value = singleRecord.BatchControl.Value.ToString();
                        //grdInventory.Columns[6].Visible = singleRecord.BatchControl.Value;
                        //grdInventory.Columns[7].Visible = singleRecord.BatchControl.Value;
                        //grdInventory.Columns[8].Visible = singleRecord.BatchControl.Value;
                        //grdInventory.Columns[9].Visible = singleRecord.BatchControl.Value;
                        //grdInventory.Columns[10].Visible = singleRecord.BatchControl.Value;
                        if (singleRecord.BatchControl == true)
                        {
                            hBatchoption.Value = singleRecord.BatchControl.Value.ToString();
                            foreach (System.Web.UI.WebControls.ListItem li in RbnBatchControlInNewInventory.Items)
                            {
                                if (li.Text.ToLower() == "yes")
                                {
                                    li.Selected = true;
                                    DivBatchReference.Visible = true;
                                    DivBatchReferenceDetailsInNewInventory.Visible = true;
                                }
                            }
                        }
                        else
                        {
                            ...
                            foreach (System.Web.UI.WebControls.ListItem li in RbnBatchControlInNewInventory.Items)
                            {
                                if (li.Text.ToLower() == "no")
                                {
                                    li.Selected = true;
                                    DivBatchReference.Visible = false;
                                    DivBatchReferenceDetailsInNewInventory.Visible = false;
                                }
                            }
                        }
                    }
        ...
        public void UpdateBatchInAddStock()
        {
            try
            {
                if (hid_batchid.Value != "0")
                {
                    ...
                    if (bEntity.QTY != Convert.ToInt32(txtQuantity.Text.Trim()))
                    {
                        //Update inventory
                        IM = new InventoryRepository<InventoryManager>();
                        In_Ma = IM.GetAll().Where(o => o.Id == bEntity.InventoryID).FirstOrDefault();
                        int qty = int.Parse(In_Ma.Qty.ToString());
                        In_Ma.Qty = In_Ma.Qty + Convert.ToInt32(txtQuantity.Text.Trim()) - bEntity.QTY;
                        IM.Edit(In_Ma);

                        //update storage table
                        int OpeningStock = GetQtyAvailable(ProductID.ToString());
                        int Qty = int.Parse(txtQuantity.Text);
                        Qty = Qty + GetQtyAvailable(ProductID.Value.ToString()) - bEntity.QTY.Value;
                        AddInventorySiteStorageDetails(ProductID.Value, 0, 0, int.Parse(ddlsiteInNewItem.SelectedValue), Qty);
                        ...
                        In_mj.DateOrdered = DateTime.Now.Date;
                        //
                        In_mj.SiteId = int.Parse(ddlsiteInNewItem.SelectedValue);
                        In_mj.UseID = 0;
                        In_mj.ModifiedBy = sessionKeys.UID;
                        In_mj.ReasonCode = "Batch Qty modified";
                        In_mj.MdofiedDate = DateTime.Now;
                        In_mj.OpeningStock = OpeningStock;
                        In_mj.TransferQty = int.Parse(txtQuantity.Text) - bEntity.QTY;
                        In_mj.QtyChangedOrNot = true;
                        IMJ.Add(In_mj);

                        //Update batch data
                        bEntity.QTY = Convert.ToInt32(txtQuantity.Text.Trim());
                        bEntity.AddedBy = sessionKeys.UID;
                    }
                    bEntity.SiteId = Convert.ToInt32(ddlsiteStockadd.SelectedValue);
                    bEntity.Area = Convert.ToInt32(ddlareaStockadd.SelectedValue);
                    bEntity.Location = Convert.ToInt32(ddllocationStockadd.SelectedValue);
                    bEntity.Shelf = Convert.ToInt32(ddlshelfStockadd.SelectedValue);
                    bEntity.Bin = Convert.ToInt32(ddlbinStockadd.SelectedValue);

                    bRepository.Edit(bEntity);
                    //Save Custom data
                    saveBatchCustomInAddStock(bEntity.BatchRef.Value, Convert.ToInt32(ProductID));
            ...
            try
            {
                if (hid_batchid.Value != "0")
                {
                    ...
                    if (bEntity.QTY != Convert.ToInt32(txtQtyEdit.Text.Trim()))
                    {
                        ...
                        //update storage table
                        int OpeningStock = GetQtyAvailable(ProductID.ToString());
                        int Qty = int.Parse(txtQtyEdit.Text);
                        Qty = Qty + GetQtyAvailable(ProductID.Value.ToString()) - bEntity.QTY.Value;
                        AddInventorySiteStorageDetails(ProductID.Value, 0, 0, int.Parse(ddlsiteInNewItem.SelectedValue), Qty);
                        ...
                        In_mj.MdofiedDate = DateTime.Now;
                        In_mj.OpeningStock = OpeningStock;
                        In_mj.TransferQty = int.Parse(txtQtyEdit.Text) - bEntity.QTY;
                        In_mj.QtyChangedOrNot = true;
                        IMJ.Add(In_mj);
                ...
                else if (hid_batchproduct.Value != "0")
                {
                    try
                    {
                        ...
                        Inventory_Batch_Add(ProductID, Qty, true);
                        saveBatchCustom(Convert.ToInt32(txtBatchreference.Text.Trim()), Convert.ToInt32(ProductID));
                        ...
                        //update storage table
                        int OpeningStock = GetQtyAvailable(ProductID.ToString());
                        int Storage_Qty = Qty + GetQtyAvailable(ProductID.ToString());
                        AddInventorySiteStorageDetails(ProductID, 0, 0, int.Parse(ddlsiteInNewItem.SelectedValue), Storage_Qty);
                ...
                else
                {
                    ...
                    using (InventoryDataContext db = new InventoryDataContext())
                    {
                        ...
                        //var retStatus = IM.GetAll().Where(o => o.ItemDescription.ToLower() == TxtproductInNewItem.Text.Trim().ToLower() && o.PortfolioID == Convert.ToInt32(ddlcustomerInNewItem.SelectedValue)).ToList().Count();
                        if (ChkMaterialDesc(Convert.ToInt32(ddlcustomerInNewItem.SelectedValue),
                            TxtproductInNewItem.Text.Trim(), Convert.ToInt32(ddlcategoryInNewItem.SelectedValue),
                            Convert.ToInt32(ddlSubcategoryInNewItem.SelectedValue), 0, Convert.ToInt32(ddlsiteInNewItem.SelectedValue)) == false)
                        {
                            ...
                            if (inventoryID > 0)
                            {
                                ...
                                if (selectedText.ToLower() == "yes")
                                {
                                    InsertIntoNewStockOnItemCreation(int.Parse(txtQtyNewItem.Text),
                                        inventoryID, true, int.Parse(ddlsiteInNewItem.SelectedValue), int.Parse(ddlareaInNewItem.SelectedValue), int.Parse(ddlLocationInNewItem.SelectedValue),
                                        int.Parse(ddlShelfInNewItem.SelectedValue), int.Parse(ddlBinInNewItem.SelectedValue));
                                    //Batch reference
                                    ib = Inventory_Batch_Add(inventoryID, Convert.ToInt32(txtQtyNewItem.Text.Trim()), true);
                                }
                                else if (selectedText.ToLower() == "no")
                                {
                                    InsertIntoNewStockOnItemCreation(int.Parse(txtQtyNewItem.Text),
                                        inventoryID, false, int.Parse(ddlsiteInNewItem.SelectedValue), int.Parse(ddlareaInNewItem.SelectedValue), int.Parse(ddlLocationInNewItem.SelectedValue),
                                        int.Parse(ddlShelfInNewItem.SelectedValue), int.Parse(ddlBinInNewItem.SelectedValue));
                                    //Batch reference
                                    ib = Inventory_Batch_Add(inventoryID, Convert.ToInt32(txtQtyNewItem.Text.Trim()), false);
                                }
                                //Save Custome Data
                        ...
                        {
                            lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                            lblInventoryAddMsg.Text = "Product already exists.";
                            mdlpopupForNewItem.Show();
                        }
                    }
                }
        ...
        public void InsertIntoNewStockOnItemCreation(int Qty, int ProductId, bool BatchControl, int SiteId, int AreaId, int LocationId, int shelfId, int Binid)
        {
            try
            {
                INS = new InventoryRepository<Inventory_New_Stock>();
                In_ns = new Inventory_New_Stock();
                In_ns.ProductId = ProductId;
                In_ns.SiteId = SiteId;
                In_ns.AreaId = AreaId;
                In_ns.LocationId = LocationId;
                In_ns.shelfId = shelfId;
                In_ns.Binid = Binid;
                In_ns.Qty = Qty;
                In_ns.Userid = sessionKeys.UID;
                In_ns.BatchControl = BatchControl;              
                INS.Add(In_ns);
            }
            catch (Exception ex)
            {
        ...
        public void BindBatchPlaceholderFields(Inventory_Batch bEntity, string CustomFieldsClear = null)
        {
            IUCD = new InventoryRepository<InventoryManager_UsageCustomData>();
            IGFC = new InventoryRepository<GridFieldConfigurator>();
            var strs = new string[] { "Qty Used", "Requester", "Project", "Status", "Notes" };
            ...
            if (clist.Count > 0)
            {
                ...
                BindBatchPlaceholder(clist, bclist, CustomFieldsClear);
            }
        }
        public void BindBatchPlaceholderFieldsInAddStock(Inventory_Batch bEntity, string CustomFieldsClear = null)
        {
            IUCD = new InventoryRepository<InventoryManager_UsageCustomData>();
            IGFC = new InventoryRepository<GridFieldConfigurator>();
            var strs = new string[] { "Qty Used", "Requester", "Project", "Status", "Notes" };
            ...
            {
                pnlBatch.Visible = true;
                BindBatchPlaceholderInStock(clist, bclist, CustomFieldsClear);
            }
        }


        public void BindBatchPlaceholder(List<GridFieldConfigurator> clist, List<Inventory_BatchCustomData> bcList, string BacthCustomData = null)
        {
            try
            {
                var strs = new string[] { "SCB PO# Reference", "Vendor Order #", "Pipeline Reference", "Project Reference" };
                bool Isfirsttime = false;
                ...
                if (clist != null)
                {
                    ...
                    foreach (var c in clist)
                    {
                        ...
                        {
                            if (strs.Contains(c.DisplayName))
                            {
        ...
        public void BindBatchPlaceholderInStock(List<GridFieldConfigurator> clist, List<Inventory_BatchCustomData> bcList, string BacthCustomData = null)
        {
            try
            {
                var strs = new string[] { "SCB PO# Reference", "Vendor Order #", "Pipeline Reference", "Project Reference" };
                ...
                if (clist != null)
                {
                    ...
                    foreach (var c in clist)
                    {
                        ...
                        if (BacthCustomData == null)
                        {
                            td.Controls.Add(GenerateTextbox("BAddStock" + c.id.ToString(), bcList.Where(o => o.CustomFieldID == c.id).FirstOrDefault() != null ? bcList.Where(o => o.CustomFieldID == c.id).FirstOrDefault().CustomFieldValue : string.Empty, string.Empty, Isfirsttime, string.Empty, false, c.DisplayName, string.Empty, string.Empty, string.Empty));
                        }
                        else
                        {
                            if (strs.Contains(c.DisplayName))
                            {
                                td.Controls.Add(GenerateTextbox("BAddStock" + c.id.ToString(), string.Empty, string.Empty, Isfirsttime, string.Empty, false, c.DisplayName, string.Empty, string.Empty, string.Empty));
                            }
                            else
                            {
                                td.Controls.Add(GenerateTextbox("BAddStock" + c.id.ToString(), bcList.Where(o => o.CustomFieldID == c.id).FirstOrDefault() != null ? bcList.Where(o => o.CustomFieldID == c.id).FirstOrDefault().CustomFieldValue : string.Empty, string.Empty, Isfirsttime, string.Empty, false, c.DisplayName, string.Empty, string.Empty, string.Empty));
                    ...
                    PlaceHolderDivInAddItems.Controls.Add(tbl);
                }
        ...
        public void BindBatchPlaceholderFieldsInAddStock()
        {
            IUCD = new InventoryRepository<InventoryManager_UsageCustomData>();
            IGFC = new InventoryRepository<GridFieldConfigurator>();
            var strs = new string[] { "Qty Used", "Requester", "Project", "Status", "Notes" };
            ...
            if (clist.Count > 0)
            {
                BindBatchPlaceholderInAddStock(clist);
            }
        }
        ...
        public void BindBatchPlaceholderInAddStock(List<GridFieldConfigurator> clist)
        {
            try
            {
                bool Isfirsttime = false;
                ...
                if (clist != null)
                {
                    ...
                    PlaceHolderBatchFiledsInAddStock.Controls.Add(tbl);
                }
        ...
        public void saveBatchCustomInAddStock(int batchid, int inventoryID)
        {
            try
            {
                ViewState["bstate"] = 2;
                ...
                if (batchDetails != null)
                {
                    ...
                    foreach (var v in clist)
                    {
                        if (PlaceHolderBatchFiledsInAddStock.FindControl("BAddStock" + v.id.ToString()) != null)
                        {
                            var txt = PlaceHolderBatchFiledsInAddStock.FindControl("BAddStock" + v.id.ToString()) as TextBox;
                            //insert data 
                            if (dlist.Where(o => o.CustomFieldID == v.id).Count() == 0)
                            {
        ...
        public Inventory_Batch Inventory_Batch_Add(int inventoryID, int Qty, bool BatchControl)
        {
            batchRepository = new InventoryRepository<InventoryMgt.Entity.Inventory_Batch>();
            Inventory_Batch ib = new Inventory_Batch();
            ib.InventoryID = inventoryID;
            ...
            ib.BatchDisplayName = "Batch Reference " + ibref.ToString();
            ib.AddedBy = sessionKeys.UID;
            ib.SiteId = Convert.ToInt32(!string.IsNullOrEmpty(ddlsiteInNewItem.SelectedValue) ? ddlsiteInNewItem.SelectedValue : "0");
            ib.Area = Convert.ToInt32(!string.IsNullOrEmpty(ddlareaInNewItem.SelectedValue) ? ddlareaInNewItem.SelectedValue : "0");
            ib.Location = Convert.ToInt32(!string.IsNullOrEmpty(ddlLocationInNewItem.SelectedValue) ? ddlLocationInNewItem.SelectedValue : "0");
            ib.Shelf = Convert.ToInt32(!string.IsNullOrEmpty(ddlShelfInNewItem.SelectedValue) ? ddlShelfInNewItem.SelectedValue : "0");
            ib.Bin = Convert.ToInt32(!string.IsNullOrEmpty(ddlBinInNewItem.SelectedValue) ? ddlBinInNewItem.SelectedValue : "0");
            ib.BatchControl = BatchControl;
            batchRepository.Add(ib);
            return ib;
        }
        ...
        public void BindFilteredBatchGrid(string Productid)
        {
            try
            {
                ...
                bList = bList.Where(o => o.BatchControl == true).ToList();
                ...
                foreach (var d in bList)
                {
                    ...
                    if (plist.Count == 0)
                    {
                        ...
                        datarw[5] = d.Area.HasValue ? (d.Area.Value > 0 ? (areaList.Where(o => o.id == d.Area.Value).FirstOrDefault().Name) : string.Empty) : string.Empty;
                        datarw[6] = d.Location.HasValue ? (d.Location.Value > 0 ? (locationList.Where(o => o.id == d.Location.Value).FirstOrDefault().Name) : string.Empty) : string.Empty;
                        datarw[7] = d.Shelf.HasValue ? (d.Shelf.Value > 0 ? (shelfList.Where(o => o.id == d.Shelf.Value).FirstOrDefault().Name) : string.Empty) : string.Empty;
                        datarw[8] = d.Bin.HasValue ? (d.Bin.Value > 0 ? (binList.Where(o => o.id == d.Bin.Value).FirstOrDefault().Name) : string.Empty) : string.Empty;
                        batchid = d.ID;
                    }
                    else
                    {
                ...
                if (dt.Rows.Count > 0)
                {
                    DivSearchforaBatch.Visible = true;
                    PanelFilterdBatchGrid.Visible = true;
                }
                else
                {
                    DivSearchforaBatch.Visible = false;
                    PanelFilterdBatchGrid.Visible = false;
                }
                GetDefaultBatchData(Productid);
            }
            catch (Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
        ...
        public void GetDefaultBatchData(string ProductId)
        {
            try
            {
                int Sumvalue = 0;
                ...
                if (bList != null)
                {
                    ...
                    TxtDefaultOpeningQTY.Text = bList.QTY.Value.ToString();
                    TxtDefaultAvailableQTY.Text = Sumvalue.ToString();
                    lblDefaultBatchId.Text = bList.ID.ToString();
                }
                else
                {
                    lblDefaultBatchId.Text = "0";
                    TxtDefaultOpeningQTY.Text = "0";
                    TxtDefaultAvailableQTY.Text = "0";
                }
            }
            catch (Exception ex)
            {
        ...
        public DataTable BindBatchsInmainGrid(int productid)
        {
            DataTable dt = new DataTable();
            try
            {
        ...
        public void BindBatchGrid()
        {
            try
            {
                ...
                //var InList = IMPSD.GetAll().Where(a => a.SectionType == "PROJECT" && a.ProductId == int.Parse(ddlProduct.SelectedValue)).ToList();
                var productid = int.Parse(ddlProduct.SelectedValue);
                var bList = ibatchRepository.GetAll().Where(o => o.InventoryID == productid && o.BatchControl == true).ToList();
                //Get location Data
                var areaList = ibatchArea.GetAll().Where(o => bList.Select(p => p.Area).ToArray().Contains(o.id)).ToList();
                var locationList = ibatchLocation.GetAll().Where(o => bList.Select(p => p.Location).ToArray().Contains(o.id)).ToList();
                var shelfList = ibatchShelf.GetAll().Where(o => bList.Select(p => p.Shelf).ToArray().Contains(o.id)).ToList();
                ...
                if (dt.Rows.Count > 0)
                {
                    DivBatchSummaryTitle.Visible = true;
                    DivBatchSummary.Visible = true;
                }
                else
                {
                    DivBatchSummaryTitle.Visible = false;
                    DivBatchSummary.Visible = false;
                }
        ...
        public void BindRecordToBatchFileds(int BatchId, string CustomFieldsClear = null)
        {
            try
            {
                var bRepository = new InventoryRepository<Inventory_Batch>();
                var bEntity = bRepository.GetAll().Where(o => o.ID == BatchId).FirstOrDefault();
                ...
                ph_batchcolumns.Controls.Clear();
                BindBatchPlaceholderFields(bEntity, CustomFieldsClear);
            }
            catch (Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
        ...
        public void BindRecordToBatchFiledsInAddStock(int BatchId, string CustomFieldsClear = null)
        {
            try
            {
                var bRepository = new InventoryRepository<Inventory_Batch>();
                var bEntity = bRepository.GetAll().Where(o => o.ID == BatchId).FirstOrDefault();
                //txtBatchreference.Text = (bEntity.BatchRef.HasValue ? bEntity.BatchRef.Value : 0).ToString();
                txtQuantity.Text = (bEntity.QTY.HasValue ? bEntity.QTY.Value : 0).ToString();
                lblproductName.Text = bEntity.InventoryManager.ItemDescription;
                //Bind area dropdowns
                BindSiteInPopup();
                ddlsiteStockadd.SelectedIndex = ddlsiteStockadd.Items.IndexOf(ddlsiteStockadd.Items.FindByValue((bEntity.SiteId.HasValue ? bEntity.SiteId.Value : 0).ToString()));
                BindAreaInPopup(bEntity.InventoryManager.PortfolioID.HasValue ? bEntity.InventoryManager.PortfolioID.Value : 0);
                ddlareaStockadd.SelectedIndex = ddlareaStockadd.Items.IndexOf(ddlareaStockadd.Items.FindByValue((bEntity.Area.HasValue ? bEntity.Area.Value : 0).ToString()));
                BindLocationInpopUp((bEntity.Area.HasValue ? bEntity.Area.Value : 0));
                ddllocationStockadd.SelectedIndex = ddllocationStockadd.Items.IndexOf(ddllocationStockadd.Items.FindByValue((bEntity.Location.HasValue ? bEntity.Location.Value : 0).ToString()));
                BindShelfInpopUp(bEntity.Location.HasValue ? bEntity.Location.Value : 0);
                ddlshelfStockadd.SelectedIndex = ddlshelfStockadd.Items.IndexOf(ddlshelfStockadd.Items.FindByValue((bEntity.Shelf.HasValue ? bEntity.Shelf.Value : 0).ToString()));
                BindBinInpopUp(bEntity.Shelf.HasValue ? bEntity.Shelf.Value : 0);
                ddlbinStockadd.SelectedIndex = ddlbinStockadd.Items.IndexOf(ddlbinStockadd.Items.FindByValue((bEntity.Bin.HasValue ? bEntity.Bin.Value : 0).ToString()));

                PlaceHolderBatchFiledsInAddStock.Controls.Clear();
                BindBatchPlaceholderFieldsInAddStock(bEntity, CustomFieldsClear);
                ...
                BatchControlRblist.Enabled = false;
            }
            catch (Exception ex)
            {
               LogExceptions.WriteExceptionLog(ex);
        ...
        protected void GridBatch_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "Edit1")
                {
                    ...
                    BindRecordToBatchFiledsInAddStock(id);
                    //Add_Panel_Inventory(false);
                    ////SettingDataToPlaceHolderInPopUp(id);
                   ...
                   // DivBatchReference.Visible = false;
                    mdlpopupaddnewstock.Show();
                    btnSubmitInPopUp.Visible = false;
                    BtnUpdateBatchDetails.Visible = true;
                }

                if (e.CommandName == "Edit1Old")
                {
                    int id = Convert.ToInt32(e.CommandArgument.ToString());
                    //set batchid
                    hid_batchid.Value = id.ToString();
                    BindRecordToBatchFileds(id);
                    Add_Panel_Inventory(false);
                    //SettingDataToPlaceHolderInPopUp(id);
                    btnSaveInNewItem.Text = "Update";
        ...
        protected void ddlcategoryInSearch_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(ddlcategoryInSearch.SelectedValue))
            {
                BindSubCategory_InSearch(Convert.ToInt32(ddlcategoryInSearch.SelectedValue), sessionKeys.PortfolioID);
                BindProductInSearch();
                InventoryGridBind();
            }
        }
        protected void GridBatchInMainGrid_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
        ...
        protected void GridusageInMainGrid_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                IGFC = new InventoryRepository<GridFieldConfigurator>();
        ...
        public string GetLableIdInUsage(string In_Id)
        {
            string Id = "DivPleaseWaitMsgInUsage" + In_Id;
            return Id;
        }
        public string GetLableIdInBatch(string In_Id)
        {
            string Id = "DivPleaseWaitMsgInBatch" + In_Id;
            return Id;
        }
        public void ReorderStockItemMail(int InventoryId)
        {
            try
            {
                using (InventoryDataContext dd = new InventoryDataContext())
                {
                    ...
                    {
                        var InventoryRecord = dd.InventoryManagers.Where(a => a.Id == Convert.ToInt32(InventoryId)).FirstOrDefault();
                        string QtyAvailable = getActualStackLaevelInMainGrid(InventoryId.ToString()).ToString();
                        if (InventoryRecord.ReOrderLevel > 0 && InventoryRecord.ReOrderLevel >= Convert.ToInt32(QtyAvailable))
                        {
                            string fromEmailId = Deffinity.systemdefaults.GetFromEmail();
                            string subject = string.Format("Reorder Stock Item");

                            var x = dd.Inventory_DistributionLists.Where(m => m.CustomerID == InventoryRecord.PortfolioID.Value).Select(r => r).ToList();
                            ...
                            string body1 = em.ReadFile("~/WF/Inventory/EmailTemplates/Inventory_EmailTemplate.html");
                            ...
                            body1 = body1.Replace("[ref]", Deffinity.systemdefaults.GetWebUrl());
                            body1 = body1.Replace("[Description]", InventoryRecord.ItemDescription);
                            body1 = body1.Replace("[QtyAvailable]", QtyAvailable);
                            body1 = body1.Replace("[datetime]", DateTime.Now.ToShortDateString());
                            body1 = body1.Replace("[Reorderlevel]", InventoryRecord.ReOrderLevel.ToString());

                            body1 = body1.Replace("[mail_head]", subject);

                            foreach (var users in result)
                            {
                                string body = body1;
                                body = body.Replace("[pm]", users.ContractorName);

                                em.SendingMail(fromEmailId, subject, body, users.EmailAddress);
                            }
                        }
                    }
                }
        ...
        protected void ChcekFromBacthControl_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (ChcekFromBacthControl.Checked == true)
                {
                    DivNonBatchControlValues.Visible = true;
                    PanelFilterdBatchGrid.Visible = false;
                    DivSearchforaBatch.Visible = false;
                    mdlpopupinGrid.Show();
                }
                else
                {
                    PanelFilterdBatchGrid.Visible = true;
                    DivSearchforaBatch.Visible = true;
                    DivNonBatchControlValues.Visible = false;
                    mdlpopupinGrid.Show();
                }
            }
            catch (Exception ex)
        ...
        public bool BatchVisibilityInMainGrid(string InventoryId)
        {
            bool BatchVisible = true;
            try
            {
                using (InventoryDataContext Idc = new InventoryDataContext())
                {
                    var x = Idc.Inventory_Batches.Where(a => a.InventoryID == int.Parse(InventoryId) && a.BatchControl == true).FirstOrDefault();
                    if (x == null)
                    {
                        BatchVisible = false;
                    }
        ...
        protected void BatchControlRblist_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                foreach (System.Web.UI.WebControls.ListItem li in BatchControlRblist.Items)
                {
                    if (li.Selected == true)
                    {
                        ...
                        else
                        {
                            mdlpopupaddnewstock.Show();
                            PlaceHolderBatchFiledsInAddStock.Controls.Clear();
                            PlaceHolderDivInAddItems.Visible = false;
                        }
                    }
                }
                mdlpopupaddnewstock.Show();
        ...
        protected void RbnBatchControlInNewInventory_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                foreach (System.Web.UI.WebControls.ListItem li in RbnBatchControlInNewInventory.Items)
        ...
        protected void LnkBtnCategoryAddInNewInventory_Click(object sender, EventArgs e)
        {
            try
            {
                DivCategoryInNewInventory.Visible = false;
                DivCategoryAddEditInNewInventory.Visible = true;
                txtCategoryInNewInventory.Text = string.Empty;
                HiddenFieldCategoryValue.Value = string.Empty;
            }
            catch (Exception ex)
            {
                LogExceptions.WriteExceptionLog(ex);
        ...
        protected void LnkBtnCategoryEditInNewInventory_Click(object sender, EventArgs e)
        {
            try
            {
                //category edit in New Inventory
                if (ddlcategoryInNewItem.SelectedValue != "0")
                {
                    txtCategoryInNewInventory.Text = ddlcategoryInNewItem.SelectedItem.Text;
                    DivCategoryInNewInventory.Visible = false;
                    DivCategoryAddEditInNewInventory.Visible = true;
                    HiddenFieldCategoryValue.Value = ddlcategoryInNewItem.SelectedValue;
                }
                else
                {
                    lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                    lblInventoryAddMsg.Text = "Please select category";
                }
            }
            catch (Exception ex)
            {
        ...
        protected void lnkBtnCategoryCancelInInventory_Click(object sender, EventArgs e)
        {
            try
            {
                HiddenFieldCategoryValue.Value = string.Empty;
                DivCategoryInNewInventory.Visible = true;
                DivCategoryAddEditInNewInventory.Visible = false;
                txtCategoryInNewInventory.Text = string.Empty;
            }
            catch (Exception ex)
            {
        ...
        protected void lnkSubmitCategoryInNewInventory_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtCategoryInNewInventory.Text.Trim() != string.Empty)
                {
                    if (!string.IsNullOrEmpty(HiddenFieldCategoryValue.Value))
                    {
                        int msg = UpdateCategory(int.Parse(HiddenFieldCategoryValue.Value), txtCategoryInNewInventory.Text.Trim(), 0);
                        if (msg == 1)
                        {
                            lblInventoryAddMsg.Visible = true;
                            lblInventoryAddMsg.ForeColor = System.Drawing.Color.Green;
                            lblInventoryAddMsg.Text = "Category updated successfully";

                            txtCategoryInNewInventory.Text = string.Empty;
                            BindCategoryInNewItem(sessionKeys.PortfolioID);
                            ddlcategoryInNewItem.SelectedValue = HiddenFieldCategoryValue.Value;
                            BindSubCategoryInNewItem(int.Parse(ddlcategoryInNewItem.SelectedValue));
                        ...
                        else
                        {
                            lblInventoryAddMsg.Visible = true;
                            lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                            lblInventoryAddMsg.Text = "Please check category name already exists.";
                        }
                    }
                    else
                    {
                        SqlCommand comm_add = new SqlCommand("Deffinity_InsertServiceCategory", conn);
                        comm_add.CommandType = CommandType.StoredProcedure;
                        comm_add.Parameters.Add("@CategoryName", SqlDbType.NVarChar).Value = txtCategoryInNewInventory.Text.Trim();
                        comm_add.Parameters.Add("@PortfolioID", SqlDbType.Int).Value = sessionKeys.PortfolioID;
                        comm_add.Parameters.Add("@PageType", SqlDbType.Int).Value = PageType;
                        comm_add.Parameters.Add("@VendorID", SqlDbType.Int).Value = VendorID;
                        SqlParameter outCategoryID = new SqlParameter("@CategoryID", SqlDbType.Int);
                        ...
                        try
                        {
                            ...
                            if (i > 0)
                            {
                                lblInventoryAddMsg.Visible = true;
                                lblInventoryAddMsg.ForeColor = System.Drawing.Color.Green;
                                lblInventoryAddMsg.Text = "Category added successfully";

                                txtCategoryInNewInventory.Text = string.Empty;
                                BindCategoryInNewItem(sessionKeys.PortfolioID);
                                ddlcategoryInNewItem.SelectedValue = comm_add.Parameters["@CategoryID"].Value.ToString();

                                BindSubCategoryInNewItem(int.Parse(comm_add.Parameters["@CategoryID"].Value.ToString()));
                            ...
                            else
                            {
                                lblInventoryAddMsg.Visible = true;
                                lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                                lblInventoryAddMsg.Text = "Error occured while inserting";
                            }
                        }
                        catch (Exception ex)
                        {
                ...
                {
                    lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                    lblInventoryAddMsg.Text = "Please enter category name";
                }
            }
            catch (Exception ex)
            {
        ...
        protected void LnkBtnSubCategoryAddInNewInventory_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlcategoryInNewItem.SelectedValue != "0")
                {
                    DivSubCategoryAddEditInNewInventory.Visible = true;
                    DivSubCategoryInNewInventory.Visible = false;

                    txtSubCategoryInNewInventory.Text = string.Empty;
                    HiddenFieldSubCategoryValue.Value = string.Empty;
                }
                else
                {
                    lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
        ...
        protected void LnkBtnSubCategoryEditInNewInventory_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlSubcategoryInNewItem.SelectedValue != "0")
                {

                    txtSubCategoryInNewInventory.Text = ddlSubcategoryInNewItem.SelectedItem.Text;
                    DivSubCategoryAddEditInNewInventory.Visible = true;
                    DivSubCategoryInNewInventory.Visible = false;
                    HiddenFieldSubCategoryValue.Value = ddlSubcategoryInNewItem.SelectedValue;

                }
                else
                {
                    lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                    lblInventoryAddMsg.Text = "Please select sub category";
                }
            }
            catch (Exception ex)
            {
        ...
        protected void lnkSubmitSubCategoryInNewInventory_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtSubCategoryInNewInventory.Text.Trim() != string.Empty)
                {
                    if (!string.IsNullOrEmpty(HiddenFieldSubCategoryValue.Value))
                    {
                        int msg = UpdateCategory(int.Parse(HiddenFieldSubCategoryValue.Value), txtSubCategoryInNewInventory.Text.Trim(), 1);
                        if (msg == 1)
                        {
                            lblInventoryAddMsg.Visible = true;
                            lblInventoryAddMsg.ForeColor = System.Drawing.Color.Green;
                            lblInventoryAddMsg.Text = "Sub category updated successfully";

                            BindSubCategoryInNewItem(Convert.ToInt32(ddlcategoryInNewItem.SelectedItem.Value.ToString()));

                            ddlSubcategoryInNewItem.SelectedValue = HiddenFieldSubCategoryValue.Value;
                            HiddenFieldSubCategoryValue.Value = string.Empty;
                            txtSubCategoryInNewInventory.Text = string.Empty;

                            DivSubCategoryAddEditInNewInventory.Visible = false;
                            DivSubCategoryInNewInventory.Visible = true;
                        ...
                        else
                        {
                            lblInventoryAddMsg.Visible = true;
                            lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                            lblInventoryAddMsg.Text = "Please check sub category name already exists";
                        }
                    }
                    else
                    {
                        SqlCommand comm_add = new SqlCommand("Deffinity_InsertServiceSubCategory", conn);
                        comm_add.CommandType = CommandType.StoredProcedure;
                        comm_add.Parameters.Add("@CategoryName", SqlDbType.NVarChar).Value = txtSubCategoryInNewInventory.Text.Trim();
                        comm_add.Parameters.Add("@PortfolioID", SqlDbType.Int).Value = sessionKeys.PortfolioID;
                        comm_add.Parameters.Add("@MasterID", SqlDbType.Int).Value = Convert.ToInt32(ddlcategoryInNewItem.SelectedValue);
                        comm_add.Parameters.Add("@PageType", SqlDbType.Int).Value = PageType;
                        comm_add.Parameters.Add("@VendorID", SqlDbType.Int).Value = VendorID;
                        SqlParameter outCategoryID = new SqlParameter("@SubCategoryID", SqlDbType.Int);
                        outCategoryID.Direction = ParameterDirection.Output;
                        ...
                        try
                        {
                            ...
                            if (i > 0)
                            {
                                lblInventoryAddMsg.Visible = true;
                                lblInventoryAddMsg.ForeColor = System.Drawing.Color.Green;
                                lblInventoryAddMsg.Text = "Sub category added successfully";
                                //BindCategoryInNewItem(sessionKeys.PortfolioID);
                                //ddlcategoryInNewItem.SelectedValue = HiddenFieldCategoryValue.Value;
                                BindSubCategoryInNewItem(Convert.ToInt32(ddlcategoryInNewItem.SelectedItem.Value.ToString()));

                                ddlSubcategoryInNewItem.SelectedValue = comm_add.Parameters["@SubCategoryID"].Value.ToString();
                                txtSubCategoryInNewInventory.Text = string.Empty;
                            ...
                            else if (i == -1)
                            {
                                lblInventoryAddMsg.Visible = true;
                                lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                                lblInventoryAddMsg.Text = "Sub Category Already Exists";
                            }
                            else
                            {
                                lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                ...
                {
                    lblInventoryAddMsg.ForeColor = System.Drawing.Color.Red;
                    lblInventoryAddMsg.Text = "Please enter sub category name";
                }
            }
            catch (Exception ex)
            {
        ...
        protected void lnkBtnSubCategoryCancelInInventory_Click(object sender, EventArgs e)
        {
            try
            {
                DivSubCategoryAddEditInNewInventory.Visible = false;
        ...
        protected void btnViewAll_Click(object sender, EventArgs e)
        {
            try
            {
                grdInventoryNew.AllowPaging = false;
                InventoryGridBind();
            }
            catch (Exception ex)
            {
        ...
        protected void ddlProductInSearch_SelectedIndexChanged(object sender, EventArgs e)
        {
            try 
            {
                BindProductInSearch();
        ...
        protected void grdInventoryNew_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            //try
            //{
            //    if (e.CommandName == "MoreOptions")
        ...
        protected void grdInventoryNew_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    Label lblGridInventoryId = ((Label)e.Row.FindControl("lblGridInventoryId"));
                    GridView GridInner = ((GridView)e.Row.FindControl("grdInventory"));
                    if (GridInner != null)
                    {
                        InventoryInnerGridBind(lblGridInventoryId.Text, GridInner);
                    }
                }
            }
            catch (Exception ex)
